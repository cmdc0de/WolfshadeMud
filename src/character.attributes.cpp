...
//player file
fstream CCharacterAttributes::m_PlayerFile(PLAYER_FILES,
...
{
	NEWS.Empty();
	CAscii NewsFile(NEWSFILE,ios::in | ios::nocreate);
	if(NewsFile)
	{
...
{
	CAscii MotdFile(MOTDFILE,ios::in | ios::nocreate);
	if(MotdFile)
	{
...
{
	MudLog << "Saved Player size is: " << sizeof(sSaveChar) << endl;
	...
	//Seek begining
	m_PlayerFile.seekg(0,ios::end);
	long lSize = m_PlayerFile.tellg();
	...
	{
		//reset file pos
		m_PlayerFile.seekg(0,ios::beg);
		sSaveChar SaveChar;
		long lFilePos = 0;
		int nCounter = 0;
		while(m_PlayerFile.good() && (lSize!=(lFilePos = m_PlayerFile.tellg())))
		{
			m_PlayerFile.read((char *)&SaveChar,sizeof(sSaveChar));
...
void CCharacterAttributes::RemoveDeletedChars()
{
	...
	else if(lSize!=0)
	{
		...
		m_PlayerFile.open(PLAYER_FILES,ios::binary | ios::in | ios::out);
		m_PlayerFile.seekg(0,ios::beg);
		int nDeleted = 0;
		for(i = 0;i<(int)lNumOfPlayers;i++)
		{
			...
			{
				m_PlayerFile.write((char *)&pSaveChars[i],sizeof(sSaveChar));
			}
			else
			{
				nDeleted++;
...
bool CCharacterAttributes::DeleteChar()
{
	...
	if(m_PlayerFilePos.Lookup(m_strName,lFilePos))
	{
		...
		m_PlayerFile.seekg(lFilePos,ios::beg);
		m_PlayerFile.write((char *)&SaveChar,sizeof(sSaveChar));
		m_PlayerFile.flush();
		...
		str.Format("%s%s",SPELL_SAVE_PREFIX,(char *)m_strName);
		remove(str);
		return true;
	}
	return false;
...
bool CCharacterAttributes::IsPlayerSaved()
{
	...
	m_PlayerFile.seekg(lPos,ios::beg);
	sSaveChar SaveChar;
	m_PlayerFile.read((char *)&SaveChar,sizeof(sSaveChar));
	*this=SaveChar;
	return true;
...
CCharacterAttributes::CCharacterAttributes(CMobPrototype &mob,CRoom * pPutInRoom)
{
	...
	register short int i; //to run arrays
	...
	m_strAlias.Format("%s %s",(char *)m_strAlias,GVM.GetRaceName(mob.GetRace()));
...
CCharacterAttributes::CCharacterAttributes(CCharIntermediate *ch)
{
	...
	m_strAlias.Format("%s %s",GVM.GetRaceName(m_pAttributes->GetRace()),(char *)m_strName);
...
////////////////////////////
char *CCharacterAttributes::SkillWording(short nSkilled)
{
	if(nSkilled > 90)
		return "master";
