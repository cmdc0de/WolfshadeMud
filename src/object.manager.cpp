...
{
	ofstream CorpseFile(CORPSE_FILE,ios::binary|ios::out);
	if(!CorpseFile)
	{
...
{
	ifstream CorpseFile(CORPSE_FILE,ios::binary|ios::in|ios::nocreate);
	if(!CorpseFile)
	{
...
void CObjectManager::BootObjects()
{
	...
	CAscii Index(OBJINDEX,ios::in|ios::nocreate);
	if(!Index)
	{
		Error.Format("File would not open:  %s", (char *)OBJINDEX);
	...
	while(Index.good())
	{
		...
		CAscii CurrentFile(File1,ios::in|ios::nocreate);
		if(!CurrentFile)
		{
			Error.Format("File would not open: %s", (char *)File1);
			CError Err(CRITICAL_ERROR,Error);
			throw &Err;
		}
	...
	while(pos)
	{
		...
		if(pProto->ObjAlias.Find(strAlias)!=-1)
		{
			strToChar.Format("%s%s %d\r\n",(char *) strToChar,
				(char *)pProto->ObjName,pProto->Vnum);
		}
	}
}
			...
			else if(pObj->m_pCarriedby!=NULL)
			{
				if(pObj->IsAffectedBy(CObject::OBJ_AFFECT_CURRENTLY_WORN))
				{
					str.Format("%s%s is currently worn by %s\r\n",
						(char *)str,
						(char *)pObj->GetObjName(pCh),
						(char *)pObj->m_pCarriedby->GetName());
				}
				else
				{
