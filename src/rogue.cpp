				...
				&& m_Equipment[i]->GetMessageType() == PIERCE)
			{
				...
				if(DIE(nDieRoll) <= nSkill)
				{
					...
					else
					{
						...
						{
							if(DIE(131) <= GetSkill(SKILL_KNIFE_TURN) + m_Proficiency.CalcBonus(m_nSkillSphere) * 10)
							{	//if I turn, StabDamage is * 1.5.
						...
						if(!pTarget->IsDead())
						{
							...
							strToChar.Format("%s in %s back.\r\n",(char *)strWeaponName,(char *)pTarget->GetPronoun(pTarget->GetSex()));
...
void CRogue::DoBeg()
{
	...
	{
		strCommand.Format("give coins %s",(char *)this->GetName());
	...
	else
	{
		...
		if(DIE(100) <= 10)
		{
			...
			strCommand.Format("kill %s",(char *)this->GetName());
			pTarget->AddCommand(strCommand);
		}
		else
		{
...
void CRogue::DoCharm()
{
	...
	else if (Con < 100 - GetSkill(SKILL_CHARM))
	{
		...
		strToChar.Format("kill %s",(char *)this->GetName());
...
void CRogue::DoIntimidate(const CInterp *pInterp)
{
	...
	if(pTarget)
	{
		...
		{	//Check for intimidation, take into accout level diff
...
void CRogue::DoDirtToss(CCharacter *pTarget)
{
	if(pTarget)
	{
		...
		if(DIE(100) <= GetSkill(SKILL_DIRT_TOSS) && !pTarget->MakeSavingThrow(CAttributes::SAVE_SPELL))
		{//if unsuccesful save and skill check, add blindness to target
			//
