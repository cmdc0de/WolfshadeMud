...
public:
	...
	short int GetSize() {return AffectedSize;}
	short int GetSex() {return AffectedSex;}
	short int GetRace() {return AffectedRace;}
	short int GetClass() {return AffectedChClass;}
	bool IsRace(short nRace) {return m_pRaceReference->IsRace(nRace);}
	short int GetManaAdjustInt() {return m_pRaceReference->GetIntManaAdjust(AffectedStats[INT]);}
	short int GetManaAdjustWis() {return m_pRaceReference->GetWisManaAdjust(AffectedStats[WIS]);}
	short int GetManaAdjustPow() {return m_pRaceReference->GetPowManaAdjust(AffectedStats[POW]);}
	short int GetStartingMana() {return m_pRaceReference->GetStartingMana(AffectedStats);}
	short int GetStartingMoves() {return m_pRaceReference->GetStartingMoves(AffectedStats);}
	long GetHomeTown() {return m_pRaceReference->GetHomeTown();}
	short int GetStartingHitPoints() {return m_pRaceReference->GetConHitPointBonus(AffectedStats[CON]/10);}
	short int GetStartingAlignment() {return m_pRaceReference->GetStartingAlignment();}
	void SetClass(short int nClass) {BaseChClass = AffectedChClass = nClass;}
	void SetSex(short int nSex) {BaseSex = AffectedSex = nSex;}
	short GetACAdjust() {return m_pRaceReference->GetACAdjust(AffectedStats[AGI]);}
	short GetIntPrepTimeAdjust() {return m_pRaceReference->GetIntPrepTimeAdjust(AffectedStats[INT]);}
	short GetWisPrepTimeAdjust() {return m_pRaceReference->GetWisPrepTimeAdjust(AffectedStats[WIS]);} 
	void AdjustDex(short nAdj) {AffectedStats[DEX] = RANGE(AffectedStats[DEX]+nAdj,120,0);}
	void AdjustStr(short nAdj) {AffectedStats[STR] = RANGE(AffectedStats[STR]+nAdj,120,0);}
	void AdjustAgi(short nAdj) {AffectedStats[AGI] = RANGE(AffectedStats[AGI]+nAdj,120,0);}
	short GetShoutDistance() {return AffectedStats[CON]/5;}
	short GetDamageBonus() {return (m_pRaceReference->m_sStrength[(AffectedStats[STR]/10)].m_nDamage + AffectedStats[STRADD]/20);}
	short GetToHitBonus() {return (m_pRaceReference->m_sDexterity[(AffectedStats[DEX]/10)].m_nToHit);}
	skill_type DodgeBonus() {return ((skill_type)(m_pRaceReference->m_sAgility[(AffectedStats[AGI]/10)].m_nDodge));}
	skill_type GetParryBonus() {return ((skill_type)(m_pRaceReference->m_sDexterity[(AffectedStats[DEX]/10)].m_nParry));}
	skill_type GetRiposteBonus() {return ((skill_type)(m_pRaceReference->m_sDexterity[(AffectedStats[DEX]/10)].m_nRiposte));}
	bool TooOld() {return (CalcAge() >= m_pRaceReference->m_sInnateRace.m_nMaxAge);}
	int GetMaxWeight() {return m_pRaceReference->m_sStrength[(AffectedStats[STR]/10)].m_nWeightCarried;}
	short GetWisdom() {return AffectedStats[WIS];}
	short GetInt() {return AffectedStats[INT];}
	bool MakeSavingThrow(short nSavingThrow);
	stat_type GetStat(short nStat) {return AffectedStats[nStat];}
	short GetHeight() {return AffectedHeight;}
	short GetWeight() {return AffectedWeight;}
};
...
{
	for(register int i = 0;i<MAX_NUMBER_OF_STATS;i++)
	{
