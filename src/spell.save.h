...
public:
	...
	bool IsEmpty() const {return m_nNumber==0;}
    };
	...
	public:
		...
		sPreparingSave &operator=(const CPreparing<CasterClass>*pPreparing)
		{
			...
			strcpy(m_strSpellName,pPreparing->GetSpell()->GetSpellName());
			return *this;
		}
		bool IsEmpty() const {return m_bEmpty;}
		const char *GetName() const {return m_strSpellName;}
		long GetTimeLeft() const {return m_lTimeLeft;}
...
public:
	void AddPrepared(const CLinkList<CPrepared<CasterClass> > *pPrepLL)
	{
		...
		while(pos && i < MAX_SPELLS_PREPARED_FOR_SAVE)
        {
			...
			strcpy(m_PreparedSpells[i].m_strSpellName,pPrep->GetSpell()->GetSpellName());
            i++;
		}
    }
...
public:
	sSpellSave();
	const sPreped &GetPreparedSpell(short i) {return m_PreparedSpells[i];}
	const sPreparingSave &GetPreparingSpell(short i) {return m_PreparingSpells[i];}
	short GetMaxSpellsPrepared() {return MAX_SPELLS_PREPARED_FOR_SAVE;}
	short GetMaxSpellsPreparing() {return MAX_SPELLS_PREPARING_FOR_SAVE;}
};
