...
protected:
    ...
    short GetTotalNonGraySpheres() {
        return TOTAL_NON_GRAY_SPHERES;
    }
protected:

    virtual const CMap<CString, CSpell<CMage> *> *GetSpellTable() const {
        return &m_SpellTable;
    }

    virtual CProficiency &GetProficiency() {
    ...
    virtual int GetExpGap() {
        return m_Experience[GetLevel() + 1] - m_Experience[GetLevel()];
    }

    virtual int GetExpInLevel() {
        return m_nExperience - m_Experience[GetLevel()];
    }
...
inline short CMage::CalcNumOfSpheres() {
    short nSpheres = 0;
    for (int i = 0; i < GetTotalNonGraySpheres(); i++) {
