...
CMountainDwarfReference::~CMountainDwarfReference()
{
	//do nothing
}

CMountainDwarfReference::CMountainDwarfReference() : CRacialReference()
{
	SetAll();
	m_nSize = SIZE_SMALL;
...
void CMountainDwarfReference::SetCharisma()
{
...
void CMountainDwarfReference::SetIntellegence()
{
	...
	m_sIntellegence[8] = sIntellegence(0,0,2); //80-89
	m_sIntellegence[9] = sIntellegence(10,0,2); //90-99
	m_sIntellegence[10] = sIntellegence(20,0,3); //100-109
	m_sIntellegence[11] = sIntellegence(30,0,4); //110-119
	m_sIntellegence[12] = sIntellegence(40,0,5); //120
}

void CMountainDwarfReference::SetWisdom()
{
	...
	m_sWisdom[11] = sWisdom(30,-3,4); //110-119
	m_sWisdom[12] = sWisdom(40,-4,5); //120
}

void CMountainDwarfReference::SetPower()
{
...
void CMountainDwarfReference::SetConstitution()
{
	...
	m_sConstitution[3] = sConstitution(1,0,0); //30-39
	m_sConstitution[4] = sConstitution(1,0,0); //40-49
	m_sConstitution[5] = sConstitution(1,0,0); //50-59
	m_sConstitution[6] = sConstitution(1,0,0); //60-69
	...
	m_sConstitution[12] = sConstitution(12,6,45); //120
}

void CMountainDwarfReference::SetInnateRace()
{
	m_sInnateRace.m_nInnate_ablities = INNATE_ULTRAVISION |
		INNATE_DAY_SIGHT | INNATE_LARGE | INNATE_STRENGTH;
	m_sInnateRace.m_nMinHeight = 32;
	m_sInnateRace.m_nMaxHeight = 48;
	m_sInnateRace.m_nMinWeight = 65;
	m_sInnateRace.m_nMaxWeight = 250;
	m_sInnateRace.m_nMinAge = 50;
	m_sInnateRace.m_nMaxAge = 250;
...
void CMountainDwarfReference::SetStrength()
{
	...
	m_sStrength[5] = sStrength(0,110,0); //50-59
	m_sStrength[6] = sStrength(0,140,0); //60-69
	m_sStrength[7] = sStrength(1,170,0); //70-79
	m_sStrength[8] = sStrength(2,210,10); //80-89
	m_sStrength[9] = sStrength(3,250,20); //90-99
	m_sStrength[10] = sStrength(4,300,30); //90-99
	m_sStrength[11] = sStrength(5,350,40); //110-119
	m_sStrength[12] = sStrength(6,400,50); //120
}

void CMountainDwarfReference::SetDexterity()
{
	m_sDexterity[0] = sDexterity(0,-10,-10,-10); //0-9
	m_sDexterity[1] = sDexterity(0,0,0,0); //10-19
	...
	m_sDexterity[9] = sDexterity(10,0,0,0); //90-99
	m_sDexterity[10] = sDexterity(20,10,10,10); //90-99
	m_sDexterity[11] = sDexterity(30,20,20,20); //110-119
	m_sDexterity[12] = sDexterity(40,30,30,30); //120
}

void CMountainDwarfReference::SetAgility()
{
	m_sAgility[0] = sAgility(0,-20,-20,-30,-30); //0-9
	m_sAgility[1] = sAgility(0,-10,-10,-20,-20); //10-19
	m_sAgility[2] = sAgility(0,0,0,-10,-10); //20-29
	...
	m_sAgility[8] = sAgility(2,0,0,0,0); //80-89
	m_sAgility[9] = sAgility(3,10,10,10,10); //90-99
	m_sAgility[10] = sAgility(4,20,20,20,20); //100-109
	m_sAgility[11] = sAgility(5,30,30,30,30); //110-119
	m_sAgility[12] = sAgility(6,40,40,40,40); //120
}
