...
void CMobPrototype::VStat(CString & ToChar)
{
	...
	ToChar.Format(strFormat,(char *) MobName,GVM.GetRaceName(Race),
		GVM.GetClassName(ClassType),Alignment,Level,CCharacterAttributes::GetSexName(Sex),
		AC,Experience,Thaco,(char *) MobAlias,(char *) MobDescription,
		(char *)DefaultPosDesc);
}
...
CMobPrototype::CMobPrototype(CCharacter * pMirror)
{
	...
	DefaultPosDesc.Format("%s stands here",(char *)MobName);
...
CMobPrototype::CMobPrototype(const sUndeadInfo *p)
{
	...
	case VNUM_UNDEAD_SKELETON:
		...
		DefaultPosDesc.Format("%s stands here with &Rred glowing &Weyes&n",(char *)MobName);
	...
	case VNUM_UNDEAD_ZOMBIE:
		...
		DefaultPosDesc.Format("%s &Lstands here with its flesh rotting away&n",(char *)MobName);
		ClassType = CLASS_WARRIOR;
		NumOfDice = 2;
		DiceType = 5;
		Level = p->m_nLevel;
	...
	case VNUM_UNDEAD_SPECTRE:
		...
		MobAlias = "spectre";
		MobDescription = "&Lstands here.&n";
		DefaultPosDesc.Format("%s &Lfloats here&n",(char *)MobName);
	...
	case VNUM_UNDEAD_GHOST:
		...
		DefaultPosDesc.Format("%s &wfloats here&n",(char *)MobName);
		ClassType = CLASS_ROGUE;
		DiceType = 5;
		NumOfDice = 4;
		Level = p->m_nLevel;
	...
	case VNUM_UNDEAD_WIGHT:
		...
		DefaultPosDesc.Format("%s &rfloats here&n",(char *)MobName);
...
CMobPrototype::CMobPrototype(const sWallInfo *p)
{
	...
	DefaultPosDesc.Format(strDefaultDesc,(char *)MobName, CRoom::strDir[p->m_nDirection]);
	ClassType = CLASS_WARRIOR;
	DiceType = 2;
	Level = p->m_nLevel;
	m_nProficiencySphere = 0;
