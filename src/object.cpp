...
void CObject::Examine(const CCharacter *pLooker, CString & strExamine) {
	...
	while (pos) {
		strExamine.Format("%s%s\r\n", strExamine.cptr(), ExtraDesc.GetNext(pos)->ExtraDesc.cptr());
	}
}
...
CObject::sAffectInfo::sAffectInfo(CString strName, CString strAffectRate, CString strAffectRateRoom,
		CString strBattleRound, CString strBattleRoom, CString strEnd, CString strEndRoom,
		void (CObject::*fncRound)(short), void (CObject::*fncBattle)(short), void (CObject::*fncEnd)(short),
		int nRate) {
	m_strName = strName;
...
#define OBJNULL (void (CObject::*)(short)) NULL
	m_AffectInfo[OBJ_AFFECT_MIRAGE] = sAffectInfo("mirage", "", "", "", "", "", "", OBJNULL, OBJNULL,
			&CObject::FadeType, 0);
	m_AffectInfo[OBJ_AFFECT_CAST_MAGIC_MISSILE] = sAffectInfo("Magic missile", "", "", "", "", "", "", OBJNULL,
			&CObject::CastMagicMissile, OBJNULL, 0);
	m_AffectInfo[OBJ_AFFECT_DEATHS_MATRIARCH] = sAffectInfo("Deaths Matriarch", "", "", "", "", "", "", OBJNULL,
			&CObject::DeathsMatriarch, OBJNULL, 0);
	m_AffectInfo[OBJ_AFFECT_DIVINE_FATE] = sAffectInfo("Divine Fate",
			"&WDivine Fate does NOT like to be held by impure hands!&n\r\n",
			"&WDivine Fate does NOT like to be held by impure hands!&n\r\n",
			"&WDivine Fate &wflashes with &Wholy&w ra&Wdian&wce!&n\r\n",
			"&WDivine Fate &wflashes with &Wholy&w ra&Wdian&wce!&n\r\n", "", "", OBJNULL, &CObject::DivineFate, OBJNULL,
			1); //JOHN THIS MAKES NO SENSE TO HAVE WORDING FOR fncbeging, and fnc end but then don't call a function...boggle!
	...
	m_AffectInfo[OBJ_AFFECT_HONE_WEAPON] = sAffectInfo("Hone Weapon", "", "", "", "", "", "", OBJNULL, OBJNULL, OBJNULL,
			0);
	m_AffectInfo[OBJ_AFFECT_BOX] = sAffectInfo("Little Black Box", "", "", "", "", "", "", OBJNULL, OBJNULL, OBJNULL,
			0);
	m_AffectInfo[OBJ_AFFECT_INVISIBLE] = sAffectInfo("Invisible", "", "", "", "", "", "", OBJNULL, OBJNULL, OBJNULL, 0);
	m_AffectInfo[OBJ_AFFECT_CONTINUAL_LIGHT] = sAffectInfo("continual light", "", "", "", "", "", "", OBJNULL, OBJNULL,
			OBJNULL, 0);
}
		...
		if (m_Affects[i].HasAffect()) {
			if (!m_Affects[i].IsPermAffect() && !m_Affects[i].IsManual()) {
				if ((m_Affects[i].m_nTime -= lTimeToRemove) <= 0) {
					...
					if (!m_AffectInfo[i].m_strAffectRate.IsEmpty() && m_pCarriedby != NULL) {
						m_pCarriedby->SendToChar(m_AffectInfo[i].m_strAffectRate);
					}
					if (!m_AffectInfo[i].m_strAffectRateRoom.IsEmpty()) {
						m_pCarriedby == NULL ?
								m_pInRoom->SendToRoom(m_AffectInfo[i].m_strAffectRateRoom) :
								m_pCarriedby->GetRoom()->SendToRoom(m_AffectInfo[i].m_strAffectRateRoom, m_pCarriedby);
					}
...
void CObject::RemoveAffect(short nAffect) {
	...
	if (!m_AffectInfo[nAffect].m_strEnd.IsEmpty() && m_pCarriedby) {
...
void CObject::BlackBox(bool bRemoving, bool bSendMsg) {
	...
	if (bSendMsg) {
		m_pCarriedby->GetRoom()->SendToRoom(
				"&LThe little black box &Wtwists, &Rswirls &Land &Btransforms&L in %s's hands!&n\r\n", m_pCarriedby);
...
void CObject::DeathsMatriarch(short nAffect) {
	...
	if (m_pCarriedby != NULL && (pTarget = m_pCarriedby->IsFighting())) {
		if (DIE(100) < DEATHS_MATRIARCH_PCT) {
			m_pCarriedby->GetRoom()->SendToRoom(
					"&mDeath's Matriarch&L, carried by %s&L, &Cglows &Levilly and lashes out at %s&L!&n\r\n",
					m_pCarriedby, pTarget);
			m_pCarriedby->SendToChar(
					"&LYour &mDeath's Matriarch &Cglows &Levilly and lashes out at your target!&n\r\n");
			...
			while (nTimes--) {
				...
				if (pTarget->IsDead()) {
					m_pCarriedby->GetRoom()->SendToRoom(
							"&L%s&L's Death's Matriarch&L sucks the remaining life from %s&L!&n\r\n", m_pCarriedby,
							pTarget);
...
CString CObject::GetExtraDescriptionString(const char *key) {
	POSITION pos = ExtraDesc.GetStartPosition();
	CObjectDesc *desc = 0;
	while((desc = ExtraDesc.GetNext(pos))) {
		if(desc->KeyWord.Compare(key)) {
...
CString CObject::GetIdentifyString() {
	...
	strIdent.Format("Name: %s \r\nAlias: %s \r\nCost: %d \r\nWeight: %d \r\nType: %s \r\n%s \r\n", m_strObjName.cptr(),
			m_strObjAlias.cptr(), m_nCost, m_nWeight, strType.cptr(), strTypeInfo.cptr());
