...
CObject::CObject(CCharacter * pCh)
{
	...
	if(pCh->IsNPC())
	{
		str.Format("The corpse of %s lies here cold and bloody.",(char *)pCh->GetName());
	}
	else
	{
...
void CObject::Examine(const CCharacter *pLooker,CString & strExamine)
{
	strExamine.Format("%s%s\r\n",
		(char *)strExamine,(char *)m_strObjDesc);
	
	POSITION pos = ExtraDesc.GetStartPosition();
	while(pos)
	{
		strExamine.Format("%s%s\r\n",(char *)strExamine,
			(char *)ExtraDesc.GetNext(pos)->ExtraDesc);
	}
}
...
{
	for(register int i = 0;i<TOTAL_OBJ_AFFECTS;i++)
	{
...
void CObject::BlackBox(bool bRemoving, bool bSendMsg)
{
	...
	if(bRemoving) 
	{	//when removed put back the way it belongs
...
{
	for(register short i = 0;i<TOTAL_OBJ_AFFECTS;i++)
	{
...
(char *)m_strObjName,(char *)m_strObjAlias,m_nCost,m_nWeight,(char *)strType,(char *)strTypeInfo);
...
void CObject::Save(ofstream &s,int nWornOn, short nPos)
{
	if(!s)
	{
		CString str;
		str.Format("Can't open object file for player: %s",(char *)m_pCarriedby->GetName());
		CError Err(NON_CRITICAL_ERROR,str);
		throw &Err;
	}
	else
	{
...
{
	return (pCh->CanSee(this) ? (const char *)m_strObjName : "something");
}
