...
void CCharacter::DoCharacterLook(CCharacter *ch)
{
	...
	if(ch->IsNPC())
	{
		strLook.Format("%s\r\n%s\r\n%s [%s] has a condition of %s\r\n",(char *)ch->GetName(), (char *) ch->GetLongDescription(),
			(char *)ch->GetName(), (char *) GetSizeName(ch->m_pAttributes->GetSize()), ch->GetCondition());
	}
	else if(this->CanSeeName(ch))
	{
		strLook.Format("%s\r\n%s\r\n%s is a %s [%s] and has a condition of %s\r\n",(char *)ch->GetRaceOrName(ch), (char *) ch->GetLongDescription(),
			(char *)ch->GetName(), (char *) GVM.GetColorRaceName(ch->m_pAttributes->GetRace()),
			(char *) GetSizeName(ch->m_pAttributes->GetSize()), ch->GetCondition());
	}
	else
	{
		strLook.Format("%s\r\n%s\r\n%s [%s] has a condition of %s\r\n",
			(char *)ch->GetRaceOrName(this), 
			(char *)ch->GetLongDescription(),
			(char *)ch->GetRaceOrName(this), (char *) GetSizeName(ch->m_pAttributes->GetSize()),
			ch->GetCondition());
	}
...
void CCharacter::DoGlance()
{
	...
	strLook.Format("%s [%s] has a condition of %s\r\n",
		(char *)pTarget->GetRaceOrName(this), 
		(char *)GetSizeName(pTarget->m_pAttributes->GetSize()),
		...
		else
		{
			...
			while(pos)
			{
					...
					&& !pFollower->IsAffectedBy(AFFECT_CASTING))
				{
					...
					strToChar.Format("You follow %s\r\n",(char *)pFollower->m_Follow.m_pCharacterBeingFollowed->GetRaceOrName(this));
...
void CCharacter::SendPrompt()
{
	...
	strPrompt.Format("&g<%s%d&g/%d %s%d&g/%d %s%d&g/%d&g>&n%s",
		(char *)strHits,m_nCurrentHpts,m_nMaxHpts,
		(char *)strMana,m_nManaPts,m_nMaxManaPts,
		(char *)strMoves,m_nMovePts,m_nMaxMovePts,
	...
	if(InPosition(POS_FIGHTING) && IsFighting())
	{
		...
		if(pTank)
		{
			strPrompt.Format("%s<Tank: %s -> %s -- Target: %s -> %s>\r\n",
				(char *) strPrompt,
				(char *) pTank->GetRaceOrName(this),
				pTank->GetCondition(),
				(char *) IsFighting()->GetRaceOrName(this),
...
void CCharacter::DoOtherState()
{
	...
	case STATE_PAGING:
		...
		case 'n': case 'N':
			...
			{
				strToChar.Format("%s\r\n%s\r\n",
					(char *)m_PageInfo.m_strPage.Right(m_PageInfo.m_strPage.Find('\n',m_PageInfo.m_nReturnsSent)),
					"[b]egining, [p]revious page, [q]uit\r\n");
			}
			else
			{
				strToChar.Format("%s\r\n%s\r\n",
					(char *)m_PageInfo.m_strPage(m_PageInfo.m_strPage.Find('\n',m_PageInfo.m_nReturnsSent),nPos),
					strPageInfo);
		...
		case 'p': case 'P':
			...
			else
			{
				...
				strToChar.Format("%s\r\n%s\r\n",
					(char *)m_PageInfo.m_strPage(nPos,nPos1),
					strPageInfo);
				m_PageInfo.m_nReturnsSent-=m_nByScreenSize;
			}
			SendToChar(strToChar);
...
void CCharacter::DoAttributes()
{
	...
	strToChar.Format(strFormat,(char *)m_strName,GVM.GetColorRaceName(m_pAttributes->GetRace()),
		...
		m_pAttributes->AffectedStats[DEX],(char *)strSaves,
		...
		if(GetSkill(i)!=0)
		{
			if(this->IsNPC() || this->IsGod())
			{
				strToChar.Format("%s%-30s\t%d\r\n",(char *)strToChar,
					(char *)FindSkill(i),GetSkill(i));
			}
			else
			{
				strToChar.Format("%s%-30s\t%s\r\n",(char *)strToChar,
					(char *)FindSkill(i),SkillWording(GetSkill(i)));
			}
		}
	}
...
void CCharacter::Attack(bool bOnlyOneAttack)
{
    ...
    for(register int i = 0;i<nMaxAttacks;i++)
    {
        nAttacks = 0;
				...
				&& m_Equipment[i]->GetMessageType() == PIERCE)
			{
				...
				if(DIE(nDieRoll) <= GetSkill(SKILL_BACKSTAB))
				{
					...
					else
					{
						...
						if(!pTarget->IsDead())
						{
							strToChar.Format("%s makes a strange sound as you place %s in %s back.\r\n",(char *)pTarget->GetRaceOrName(this),(char *)strWeaponName,(char *)pTarget->GetPronoun(pTarget->GetSex()));
							SendToChar(strToChar);
							strToChar.Format("From out of nowhere %s places %s in YOUR back!\r\n",(char *)GetRaceOrName(pTarget),(char *)strWeaponName);
		...
		for(register int i=0; i < MAX_EQ_POS; i++)
		{
			if(!m_Equipment[i].IsEmpty() && m_Equipment[i].GetObject()->IsWeapon())
			{
				if(nDie <= nSkill)
				{
					...
					for(register int j = 0;j <= (nSkill/40 + 1);j++)
					{
						pTarget->Defense(CalcACWouldHit(),i,this);
						if(pTarget->IsDead())
						{
...
void CCharacter::Save(bool bDidJustDie)
{
	if(!IsNPC())
	{
		...
		ofstream ObjFile((char *)ObjDir,ios::binary | ios::out);
		if(bDidJustDie)
		{
			...
			//are already gone.
			for(register short i = 0;i<MAX_EQ_POS;i++)
			{
				if(!m_Equipment[i].IsEmpty())
				{
...
void CCharacter::DoRemove()
{
	...
	for(register int i = MAX_EQ_POS-1;i>=0;i--)
	{
		if(!m_Equipment[i].IsEmpty() &&
						...
						if(m_nPracticeSessions>0)
						{
							...
							if(HasSkillSet(pSkillInfo->GetSet()))
							{
								strToChar.Format("You learn the skill of %s from %s.\r\n",
									(char *)FindSkill(nSkill),
									(char *)pTeacher->GetRaceOrName(this));
							}
							else
							{
...
"%s\r\n";
	...
	strToChar.Format(strFormat,(char *)m_strName,m_nLevel,GVM.GetColorRaceName(GetRace()),
		GVM.GetColorClass(GetClass()),GetSexName(GetSex()),m_nCurrentHpts,m_nMaxHpts,
		m_nMovePts,m_nMaxMovePts,m_nManaPts,m_nMaxManaPts,
		(char *)strTime,
		(char *)m_CashOnHand.GetMoneyString(),
		(m_pBankCash==NULL ? "" : (char *)m_pBankCash->GetMoneyString()),
		GetPositionNames(),m_nPracticeSessions,m_fFrags,(char *)strEnchant);
	SendToChar(strToChar);
}
...
void CCharacter::DoCalledShot(const CInterp *pInterp)
{
	...
	else
	{
		...
		if(strShot.Compare("head"))
		{
			...
			if(nDie < nBonus)
			{
				strMessage.Format("You neatly decapitate %s with your shot to the head!",(char *)pTarget->GetName());
		...
		else if(strShot.Compare("leg"))
		{
			...
			if(nDie < nBonus * 10)
			{
				strMessage.Format("Your shot to the leg causes %s to fall!",(char *)pTarget->GetName());
				SendToChar(strMessage);
			...
			if(nDie + Adjust <= nSkill)
			{
				...
				if(nSkill - nDie > 90)
				{
					...
					{		
						if(!m_Equipment[i].IsEmpty() && m_Equipment[i]->IsWeapon())
						{
							for(register int j = 0;j < nSkill/30;j++)
							{
...
{
	CString str;
	str.Format("%-10s [%5d/%5d] [%5d/%5d] [%5d/%5d]",(char *)m_strName,
		m_nCurrentHpts,m_nMaxHpts,m_nManaPts,m_nMaxManaPts,m_nMovePts,m_nMaxMovePts);
	return CString(str);
}
...
void CCharacter::GetSingleItem(CObject * pObj)
{
	...
	else
	{
		strInfo.Format("%s is not take-able.\r\n",
			pObj->GetObjName());
		CError Err(MESSAGE_ONLY,strInfo);
		throw &Err;
	}
}
			...
			if((pObj = FindInInventory(strObj,nNum)))
			{
				...
				if(pTarget->Add(pObj))
				{
					...
					strToChar.Format("You recieve %s from %s.\r\n",
						pObj->GetObjName(),(char *)GetRaceOrName(pTarget));
					pTarget->SendToChar(strToChar);
...
void CCharacter::LoadObjects()
{
	...
	ifstream ObjFile(strObjFile,ios::binary | ios::in);
	//seek to the end
...
void CCharacter::LeaveFuguePlane()
{
	...
	strToRoom.Format("%s &wflies in and returns ",(char *)strGod);
	strToChar = strToRoom;
...
void CCharacter::DoTog()
{
	...
	if(strTog.IsEmpty())
	{
		...
		if(IsGod())
		{
			strToChar.Format(strGods,(char *)strToChar,
		...
		if(IsGod())
		{
			...
			else if(strTog.Compare("switchedinfo"))
			{
				...
				strToChar.Format("You will %s information set to %s when switched\r\n",
					Prefers(PREFERS_TELLS_WHEN_SWITCHED) ? "get" : "not get",(char *)GetName());
			}
...
void CCharacter::DoAsk(const CCharacter * pTarget)
{
	...
	else if(!pTarget->IsNPC())
	{
		...
		strAsk.Format("You ask %s '%s'\r\n",
			(char *)pTarget->GetRaceOrName(this),
			(char *)CurrentCommand.GetWordAfter(2,true));
		SendToChar(strAsk);
...
void CCharacter::SendToSocket(const CString &str) const 
{
	...
	if(send(pDesc->m_ConnectedSocket,(const char *)str,str.GetLength(),0)<0)
	{
...
void CCharacter::GodChange()
{
	...
	else
	{
		...
		if(strAtt.Compare("frags"))
		{
			...
			pTarget->m_fFrags = atof(strValue);
		...
		else if(strAtt.Compare("alignment"))
		{
			...
			pTarget->m_nAlignment = atoi(strValue);
		...
		else if(strAtt.Compare("name"))
		{
			...
			pTarget->m_strAlias.Format("%s %s",(char *)m_strName, GVM.GetRaceName(pTarget->GetRace()));
			pTarget->Save();
			strToChar.Format("%s has changed your name to %s\r\n",
				(char *)GetName(),
				(char *)pTarget->m_strName);
			pTarget->SendToChar(strToChar);
		...
		else if((nValue=strValue.MakeInt())==-1)
		{
			...
			strToChar.Format("You change %s's sex to %s.\r\n",
					(char *)pTarget->GetName(),(char *)strAtt);
			SendToChar(strToChar);
			strToChar.Format("%s has changed your sex to %s.\r\n",
				(char *)GetName(),(char *)strAtt);
			pTarget->SendToChar(strToChar);
			pTarget->Save();
		}
		else 
...
void CCharacter::InitializeCharacter()
{
	...
	for(register short i = 0;i<TOTAL_AFFECTS;i++)
	{
