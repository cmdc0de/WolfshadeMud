...
const char *CCharacter::CastingMsg[] = { "Casting: %s :\r\n", "Casting: %s :*\r\n", "Casting: %s :**\r\n",
		"Casting: %s :***\r\n", "Casting: %s :****\r\n", "Casting: %s :*****\r\n", "Casting: %s :******\r\n",
		"Casting: %s :*******\r\n", "Casting: %s :********\r\n", "Casting: %s :*********\r\n",
		"Casting: %s :**********\r\n", "Casting: %s :***********\r\n", "Casting: %s :************\r\n",
		"Casting: %s :*************\r\n", "Casting: %s :**************\r\n", "Casting: %s :***************\r\n",
...
void CCharacter::SendToChar(CString Message, bool bColorize) const {
	if ((!IsLinkDead() && !IsSwitched()) || (!IsLinkDead() && IsSwitched() && Prefers(PREFERS_TELLS_WHEN_SWITCHED))) {
		if (bColorize) {
...
bool CCharacter::HasCommand() {
	...
	if (this->IsNPC() && InPosition(POS_FIGHTING) && !InPosition(POS_STANDING) && CommandBufEmpty()) {
...
void CCharacter::DoCharacterLook(CCharacter *ch) {
	...
	if (ch->IsNPC()) {
		strLook.Format("%s\r\n%s\r\n%s [%s] has a condition of %s\r\n", ch->GetName().cptr(),
				ch->GetLongDescription().cptr(), ch->GetName().cptr(), GetSizeName(ch->m_pAttributes->GetSize()),
				ch->GetCondition());
	} else if (this->CanSeeName(ch)) {
		strLook.Format("%s\r\n%s\r\n%s is a %s [%s] and has a condition of %s\r\n", ch->GetRaceOrName(ch).cptr(),
				ch->GetLongDescription().cptr(), ch->GetName().cptr(),
				GVM.GetColorRaceName(ch->m_pAttributes->GetRace()), GetSizeName(ch->m_pAttributes->GetSize()),
				ch->GetCondition());
	} else {
		strLook.Format("%s\r\n%s\r\n%s [%s] has a condition of %s\r\n", ch->GetRaceOrName(this).cptr(),
				ch->GetLongDescription().cptr(), ch->GetRaceOrName(this).cptr(),
...
void CCharacter::DoGlance() {
	...
	strLook.Format("%s [%s] has a condition of %s\r\n", pTarget->GetRaceOrName(this).cptr(),
			GetSizeName(pTarget->m_pAttributes->GetSize()), pTarget->GetCondition());
...
void CCharacter::DoMove(CString strDirection, CObject *pObjBeingDraged) {
	...
	if (!this->IsAffectedBy(AFFECT_CASTING) && (pToRoom = GetRoom()->CanEnter(this, strDirection, strReverse)) != NULL) {
		...
		if ((m_nMovePts - nCost) <= 0) {
			...
			if (!IsAffectedBy(AFFECT_SNEAKING)) {
				if (pObjBeingDraged != NULL) {
					strPrep.Format("leaves %s dragging %s behind %s.\r\n", strDirection.cptr(),
							pObjBeingDraged->GetObjName(), CCharacterAttributes::GetPronoun(GetSex()));

					strToChar.Format("You leave %s dragging %s behind you.\r\n", strDirection.cptr(),
							pObjBeingDraged->GetObjName(this));
					SendToChar(strToChar);
				} else {
					strPrep.Format("%s %s.\r\n", (InPosition(POS_STANDING) ? "leaves" : "crawls"), strDirection.cptr());
			...
			while (pos) {
				...
				if (pOldRoom->IsInRoom(pFollower) && pFollower->InPosition(POS_STANDING)
						&& !pFollower->InPosition(POS_FIGHTING) && !pFollower->IsAffectedBy(AFFECT_CASTING)) {
...
void CCharacter::GetObjFromObj(CObject *pObj, CContainer *pCon) {
	...
	if (!pCon->Contains(pObj) || pObj == pCon) {
		...
		if (this->Add(pObj, false)) {
			//send messages
			CString str;
			str.Format("You get %s from %s.\r\n", pObj->GetObjName(), pCon->GetObjName());
			SendToChar(str);
			str.Format("%s gets %s from %s\r\n", "%s", pObj->GetObjName(), pCon->GetObjName());
...
void CCharacter::SendPrompt() {
	...
	strPrompt.Format("&g<%s%d&g/%d %s%d&g/%d %s%d&g/%d&g>&n%s", strHits.cptr(), m_nCurrentHpts, m_nMaxHpts,
			strMana.cptr(), m_nManaPts, m_nMaxManaPts, strMoves.cptr(), m_nMovePts, m_nMaxMovePts,
	...
	if (InPosition(POS_FIGHTING) && IsFighting()) {
		...
		if (pTank) {
			strPrompt.Format("%s<Tank: %s -> %s -- Target: %s -> %s>\r\n", strPrompt.cptr(),
					pTank->GetRaceOrName(this).cptr(), pTank->GetCondition(), IsFighting()->GetRaceOrName(this).cptr(),
					IsFighting()->GetCondition());
		} else {
			strPrompt.Format("%s<Target: %s -> %s>\r\n", strPrompt.cptr(), IsFighting()->GetRaceOrName(this).cptr(),
					IsFighting()->GetCondition());
		}
	}
	...
	case STATE_PAGING:
		...
		case 'P':
			if (m_PageInfo.m_nReturnsSent - m_nByScreenSize <= 0) {
				...
				strToChar.Format("%s\r\n%s\r\n", m_PageInfo.m_strPage(nPos, nPos1).cptr(), strPageInfo);
				m_PageInfo.m_nReturnsSent -= m_nByScreenSize;
			}
			SendToChar(strToChar);
			break;
...
void CCharacter::DoAttributes() {
	static const char * strFormat = "	            &Y--==Attributes for &W%s&Y==--\r\n"
	...
	strToChar.Format(strFormat, m_strName.cptr(), GVM.GetColorRaceName(m_pAttributes->GetRace()),
			m_pAttributes->CalcAge(), GVM.GetColorClass(m_pAttributes->GetClass()), GetSizeName(this->GetSize()),
			m_nLevel, m_nCurrentHpts, m_nMaxHpts, m_nManaPts, m_nMaxManaPts, m_nMovePts, m_nMaxMovePts,
			m_pAttributes->AffectedHeight, m_pAttributes->AffectedWeight, m_pAttributes->AffectedStats[STR],
			m_pAttributes->AffectedStats[AGI], m_pAttributes->AffectedStats[LUCK], m_pAttributes->AffectedStats[INT],
			m_pAttributes->AffectedStats[WIS], m_pAttributes->AffectedStats[CHA], m_pAttributes->AffectedStats[CON],
			m_pAttributes->AffectedStats[POW], m_pAttributes->AffectedStats[DEX], strSaves.cptr(), CalcAC(NULL),
			m_nToHit, m_nDamRoll, m_nAlignment, GetLoadWording(), m_nExperience, GVM.GetColorGod(GetGod()));
	...
	for (int i = 0; i < MAX_MUD_SKILLS; i++) {
		if (GetSkill(i) != 0) {
			if (this->IsNPC() || this->IsGod()) {
				strToChar.Format("%s%-30s\t%d\r\n", strToChar.cptr(), FindSkill(i).cptr(), GetSkill(i));
			} else {
				strToChar.Format("%s%-30s\t%s\r\n", strToChar.cptr(), FindSkill(i).cptr(), SkillWording(GetSkill(i)));
			}
		}
	}
...
void CCharacter::Defense(short int nACWouldHit, short nPos, CCharacter * pAttacker) {
	short nWeaponMsg;
	if (pAttacker->m_Equipment[nPos].IsEmpty() || !pAttacker->m_Equipment[nPos].GetObject()->IsWeapon()) {
		nWeaponMsg = PUNCH;
	} else {
			...
			if (GetSkill(SKILL_RIPOSTE)
					&& ((InPosition(POS_STANDING) && DIE(GAME_VAR_DEFENSE_RIPOSTE) <= GetSkill(SKILL_RIPOSTE))
							|| (!InPosition(POS_STANDING)
									&& DIE(GAME_VAR_DEFENSE_RIPOSTE) <= (GetSkill(SKILL_RIPOSTE) >> 3)))) {
...
short int CCharacter::CalcDamage(short nPos, CCharacter *pDefender) {
	...
	if (m_Equipment[nPos].IsEmpty() || !m_Equipment[nPos].GetObject()->IsWeapon()) {
		nDam += ROLL(m_nTypeBareHandDamDice, m_nNoBareHandDamDice) + m_nPlusBareHandDamDice
				+ (GetSkill(SKILL_BARE_HAND_FIGHTING) / 5);
		SkillUp(SKILL_BARE_HAND_FIGHTING);
		if (IsAffectedBy(AFFECT_FISTS_OF_STONE))
			nDam += 2;
	...
	if ((!(pDefender->InPosition(POS_FIGHTING))) || (!(pDefender->InPosition(POS_STANDING)))) {
		nDam <<= 1;
	}
	if (DIE(1000) <= 10) //0.5%
			{
...
void CCharacter::DoBackStab(const CInterp *pInterp, CCharacter *pTarget) {
	if (!GetSkill(SKILL_BACKSTAB)) {
		...
		for (int i = 0; i < MAX_EQ_POS; i++) {
			if (!m_Equipment[i].IsEmpty() && m_Equipment[i]->IsWeapon() && m_Equipment[i]->GetMessageType() == PIERCE) {
				bHadPiercer = true;
					...
					if (pTarget->MakeSkillSave(SKILL_BACKSTAB, 0)) {
						...
						if (!pTarget->IsDead()) {
							strToChar.Format("%s makes a strange sound as you place %s in %s back.\r\n",
									pTarget->GetRaceOrName(this).cptr(), strWeaponName.cptr(),
									pTarget->GetPronoun(pTarget->GetSex()));
							SendToChar(strToChar);
							...
							strToChar.Format("%s is suddenly very silent as you place %s in %s back.\r\n",
									pTarget->GetRaceOrName(this).cptr(), strWeaponName.cptr(),
									(char *) pTarget->GetPronoun(pTarget->GetSex()));
							SendToChar(strToChar);
							strToChar.Format(
									"From out of nowhere %s places %s in YOUR back, it is the last thing you ever feel.\r\n",
									GetRaceOrName(pTarget).cptr(), strWeaponName.cptr());
							pTarget->SendToChar(strToChar);
							strToRoom = "%s makes a strange sound as %s places ";
							strToChar.Format(
									"%s in thier back, making a strange sound, a lot of blood and a corpse.\r\n",
									strWeaponName.cptr());
							GetRoom()->SendToRoom(strToRoom += strToChar, pTarget, this);
							pInterp->KillChar(pTarget);
...
void CCharacter::DoBearHug(const CInterp *pInterp, CCharacter *pTarget) {
	if (!GetSkill(SKILL_BEAR_HUG)) {
		...
		CString strToChar;
		if (nDie <= nSkill) {
			pTarget->TakeDamage(this,
					DIE(nSkill)
							+ GetSkill(SKILL_BEAR_HUG)
									* (this->GetProficiency().CalcBonus(m_nSkillSphere) >= 0 ?
											this->GetProficiency().CalcBonus(m_nSkillSphere) : 0), true);
			if (pTarget->IsDead()) {
		...
		LagChar(
				(4 * CMudTime::PULSES_PER_BATTLE_ROUND)
						- (this->GetProficiency().CalcBonus(m_nSkillSphere)) * CMudTime::PULSES_PER_BATTLE_ROUND);
	}
}
...
int CCharacter::AdvanceLevel(bool bIncreaseLevel, bool bSendMsg, bool bIsFollower) {
	...
	if (bIncreaseLevel) {
		...
		if (!(GetLevel() % LVLS_PER_PRACTICE_SESSION) && !IsGod()) {
			m_nPracticeSessions++;
			if (bSendMsg) {
		...
		if (!(GetLevel() % LVLS_PER_PRACTICE_SESSION) && !IsGod() && !this->StatCheck(LUCK)) {
...
void CCharacter::UpDatePosition(UINT nPosition, bool bSendStr) {
	...
	case POS_STANDING:
		...
		if (InPosition(POS_RESTING) || InPosition(POS_SITTING) || InPosition(POS_RECLINING)) {
			if (bSendStr) {
	...
	case POS_SLEEPING:
		...
		} else if (InPosition(POS_RESTING) || InPosition(POS_SITTING)) {
			if (bSendStr) {
...
CObject * CCharacter::FindInInventory(CString & strItem, short nNum) {
	...
	while (pos) {
		pObj = Inventory.GetNext(pos);
		if (pObj->ByAlias(strItem) && this->CanSee(pObj)) {
			if (++nCount == nNum) {
				return pObj;
			}
		}
...
void CCharacter::EqList(const CCharacter *pLooker, CString & strEq) {
	for (int i = 0; i < MAX_EQ_POS; i++) {
		if (!m_Equipment[i].IsEmpty() && pLooker->CanSee(m_Equipment[i].GetObject())) {
...
void CCharacter::DoRemove() {
	...
	for (int i = MAX_EQ_POS - 1; i >= 0; i--) {
		if (!m_Equipment[i].IsEmpty() && (m_Equipment[i]->ByAlias(strToRemove) || bRemoveAll)) {
			bFound = true;
			...
			if (pSkillInfo->CanLearn()) {
				if (HasSkillSet(this->GetProficiency().GetNeededPriorSkillKnowledge(pSkillInfo))) {
					...
					if (CanAffordTraining(Cost, nSkill, pTeacher, true)) {
						if (m_nPracticeSessions > 0) {
							...
							if (HasSkillSet(pSkillInfo->GetSet())) {
								...
								strToChar.Format(
										"You add the proficiency of %s to your honors,\r\n as you learn the skill %s from %s.\r\n",
										strSets.cptr(), FindSkill(nSkill).cptr(), pTeacher->GetRaceOrName(this).cptr());
							}
							if (!Cost.IsEmpty()) {
					...
					strToChar.Format("You learn the skill %s from %s but only b/c your a god.\r\n",
							FindSkill(nSkill).cptr(), pTeacher->GetRaceOrName(this).cptr());
					SendToChar(strToChar);
				} else {
					this->GetProficiency().GetSkillSets(pSkillInfo->GetSet(), strSets, false);
...
void CCharacter::DoScore() {
	static const char * strFormat = "                Score information for %s\r\n"
	...
	strToChar.Format(strFormat, m_strName.cptr(), m_nLevel, GVM.GetColorRaceName(GetRace()),
			GVM.GetColorClass(GetClass()), GetSexName(GetSex()), m_nCurrentHpts, m_nMaxHpts, m_nMovePts, m_nMaxMovePts,
			m_nManaPts, m_nMaxManaPts, strTime.cptr(), m_CashOnHand.GetMoneyString().cptr(),
			(m_pBankCash == NULL ? "" : m_pBankCash->GetMoneyString().cptr()), GetPositionNames(), m_nPracticeSessions,
			m_fFrags, strEnchant.cptr());
	SendToChar(strToChar);
}
...
CString CCharacter::GetRaceOrName(const CCharacter *pCh) const {
	...
	if (nVision == SEES_NOTHING || !pCh->CanSeeChar(this)) {
		...
		CString str;
		if (tolower(strRace[0]) == 'a' || tolower(strRace[0]) == 'e' || tolower(strRace[0]) == 'i'
				|| tolower(strRace[0]) == 'o' || tolower(strRace[0]) == 'u') {
...
void CCharacter::DoReport() {
	...
	RepMsg.Format("I have %d (%d) hit points, %d (%d) mana points and %d (%d) movement points.'\r\n", m_nCurrentHpts,
			m_nMaxHpts, m_nManaPts, m_nMaxManaPts, m_nMovePts, m_nMaxMovePts);
	MsgToRoom += RepMsg;
					...
					for (int i = 0; i < MAX_EQ_POS; i++) {
						if (!m_Equipment[i].IsEmpty() && m_Equipment[i]->IsWeapon()) {
							for (int j = 0; j < nSkill / 30; j++) {
								pTarget->Defense(
										CalcACWouldHit() - this->GetProficiency().CalcBonus(m_nSkillSphere) * 10, i,
										this);
								if (pTarget->IsDead())
									j = nSkill / 30;
							}
...
void CCharacter::DoBuy(const CInterp * pInterp) {
	if (!m_pInRoom->IsShop()) {
		...
		if ((lCost = ((CShopRoom *) m_pInRoom)->GetCost(nNum)) == -1) {
			SendToChar("Buy what?\r\n");
		} else if (!((CShopRoom *) m_pInRoom)->CanPillage() && !((CShopRoom *) m_pInRoom)->IsShopKeeperInRoom()) {
...
void CCharacter::DoSell(const CInterp * pInterp) {
	...
	if (!pObj) {
		...
		CString strToChar;
		strToChar.Format("You have sold the %s for %s\r\n", (char *) pObj->GetObjName(), Price.GetMoneyString().cptr());
		SendToChar(strToChar);
		Remove(pObj);
...
CString CCharacter::GetVitalStats() {
	CString str;
	str.Format("%-10s [%5d/%5d] [%5d/%5d] [%5d/%5d]", m_strName.cptr(), m_nCurrentHpts, m_nMaxHpts, m_nManaPts,
			m_nMaxManaPts, m_nMovePts, m_nMaxMovePts);
	return CString(str);
}
	...
	if (m_nLevel != LVL_IMP) {
		...
		if (pDefender->IsDead()) {
			...
			if (!IsGod() && !pDefender->IsGod()) {
				...
				nExperience = ((pDefender->GetExp(pDefender->GetLevel())
						- pDefender->GetExp((pDefender->GetLevel()) - 1)) * (EXP_LOST_IN_DEATH)) / 100;
	...
	if ((pTarget = GetTarget(2, false))) {
		if (pTarget == this) {
			...
			try {
				...
				SendToChar(strToChar);
				strToChar.Format("%s gives you some %s.\r\n", GetRaceOrName(pTarget).cptr(), strMoney);
				pTarget->SendToChar(strToChar);
			...
			if ((pObj = FindInInventory(strObj, nNum))) {
				CString strToChar;
				if (pTarget->Add(pObj)) {
					strToChar.Format("You give %s to %s.\r\n", pObj->GetObjName(), pTarget->GetRaceOrName(this).cptr());
					SendToChar(strToChar);
					Remove(pObj);
...
void CCharacter::SkillUp(short nSkill) {
	if (GetSkill(nSkill)) {
		if (this->GetSkillInfo(nSkill)->GetMax() > GetSkill(nSkill)
				&& ((this->GetSkillInfo(nSkill)->GetPct() + MIN(this->GetLearingBonus(), 3))
						>= DIE(DIE_ROLL_FOR_SKILL_UP))) {
...
CCharacter * CCharacter::GetTarget(int nWordAfter, bool bAllowFight) {
	...
	if (strTarget.IsEmpty()) {
		...
		if (bAllowFight && IsFighting() && pTarget == IsFighting() && !IsFighting()->IsDead()) {
			...
			if (!pTarget || !CanSeeChar(pTarget) || (CanSeeInRoom(GetRoom()) == SEES_NOTHING && pTarget != this)
					|| pTarget->IsDead()) {
				return NULL;
			}
			return pTarget;
...
CObject * CCharacter::GetTarget(int nWordAfter, bool bInventory, bool &bAll) {
	...
	CObject *pTarget =
			bInventory ? FindInInventory(strTarget, nNum) : GetRoom()->FindObjectInRoom(strTarget, nNum, this);
...
void CCharacter::AddFighters(CCharacter *pTarget, bool bAddMeTo) {
	if (!pTarget->IsDead() && !this->IsDead() && pTarget != this) {
		if (!pTarget->IsFighting()) {
...
bool CCharacter::CheckAge() {
	if ((!(CMudTime::GetCurrentPulse() % CHECK_AGE_DEATH)) && m_pAttributes->TooOld() && m_pDescriptor != NULL
			&& !this->IsNPC()) {
...
void CCharacter::DoTog() {
	static const char *strPlayerHelp = "				Player tog Help\r\n"
	...
	static const char *strGodHelp = "				God tog help\r\n"
	...
	static const char *strPlayers = "			Player options:\r\n"
	...
	CString strToChar;
	if (strTog.IsEmpty()) {
		strToChar.Format(strPlayers, (int) m_nByScreenSize, strAnswer[Prefers(PREFERS_COLOR)], (int) m_nWimpy,
				strAnswer[Prefers(PREFERS_AUTO_EXIT)], strAnswer[Prefers(PREFERS_MAP_INSIDE)],
				strAnswer[Prefers(PREFERS_BRIEF)], strAnswer[Prefers(PREFERS_MAP_OUTSIDE)],
				strAnswer[Prefers(PREFERS_TELLS)], strAnswer[Prefers(PREFERS_COMPACT)], strAnswer[Prefers(PREFERS_OOC)],
				strAnswer[Prefers(PREFERS_EXTRA_RETURNS)], strAnswer[Prefers(PREFRES_NO_PAGING)]);
		if (IsGod()) {
			strToChar.Format(strGods, strToChar.cptr(), strAnswer[Prefers(PREFERS_NO_HASSLE)],
					strAnswer[Prefers(PREFERS_TELLS_WHEN_SWITCHED)], strAnswer[Prefers(PREFERS_GOD_LIGHT)],
					strAnswer[Prefers(PREFERS_GOD_SEES_ALL)], strAnswer[Prefers(PREFERS_PETITION)],
					strAnswer[Prefers(PREFERS_GOD_CHAT)], strAnswer[Prefers(PREFERS_GOD_LOG_DEATHS)],
					strAnswer[Prefers(PREFERS_GOD_LOG_ZONES)], strAnswer[Prefers(PREFERS_GOD_CONNECTION_LOG)],
					strAnswer[Prefers(PREFERS_GOD_ON_WHO)], strAnswer[Prefers(PREFERS_GOD_INVIS)]);
		}
		SendToChar(strToChar);
		...
		if (IsGod()) {
			if (strTog.Compare("no_hassle")) {
				...
				strToChar = (
						Prefers(PREFERS_NO_HASSLE) ?
								"You will no longer be hassled.\r\n" : "You will allow mobs to hassle you.\r\n");
...
void CCharacter::SendCharPage(CString str) {
	...
	if (nPos == -1 || !Prefers(PREFRES_NO_PAGING)) {
		...
		m_PageInfo.m_nReturnsSent = m_nByScreenSize;
		CString strToChar;
		strToChar.Format("%s\r\n%s\r\n", str.Left(nPos).cptr(), strPageInfo);
		SendToChar(strToChar);
	}
}
	...
	if (m_Consent.GetConsented() == pCharToBeTaught) {
		...
		if (pCharToBeTaught->CanSeeName(this) && pCharToBeTaught->CanSeeChar(this)) {
			if (IsGoodEnoughToTeach(pCharToBeTaught, nSkillToLearn, bIsSkill)) {
				bRetVal = ((GetSkill(nSkillToLearn) - pCharToBeTaught->GetSkill(nSkillToLearn))
						/ PC_TO_PC_TEACHER_PENATLY);
			} else {
...
void CCharacter::BuildPracticePriceSkills(const CCharacter * pToBeTaught, CString &str) const {
	...
	for (i = 0; i < MAX_MUD_SKILLS; i++) {
		if (this->IsGoodEnoughToTeach(pToBeTaught, i, true)) {
			...
			str.Format("%s%-30s\t%s\r\n", str.cptr(), FindSkill(i).cptr(), Cost.GetMoneyString().cptr());
		}
	}
}
	...
	if (InPosition(POS_SLEEPING)) {
	  ...
	  //we can see everyone in all conditions
		...
		if (nRoomBrightness == CRoom::ROOM_TWILIGHT) {
			...
			if (HasInnate(INNATE_ULTRAVISION) && HasInnate(INNATE_DAY_SIGHT)) {
				nCheckLight = SEES_ALL;
			}	//we if just one (ULTRA or DAYSIGHT) a chance to see
			else if (HasInnate(INNATE_ULTRAVISION) || HasInnate(INNATE_DAY_SIGHT)) {
	...
	if (pCh != NULL) {
		if (this->InPosition(POS_SLEEPING)) {
			...
			if (pCh->IsGod() && pCh->Prefers(PREFERS_GOD_INVIS)) {
				...
				bRetVal = false;
			} else if (pCh->IsAffectedBy(AFFECT_INVISIBLE) && !this->IsAffectedBy(AFFECT_DETECT_INVISIBILITY)) {
				bRetVal = false;
...
void CCharacter::DoOpen(bool bOpen) {
	...
	if (!strDir.IsEmpty() && (nDir = GetRoom()->IsKeyWordForDoor(str, strDir)) != NOWHERE) {
...
void CCharacter::DoTrack() {
	...
	if (GetSkill(SKILL_TRACK) <= 0) {
		...
		m_pTrackInfo->RemoveAllTrackTargets(this);
	} else if (!m_pTrackInfo->m_TrackList.IsEmpty() && !strTarget.IsEmpty()) {
...
void CCharacter::DoSplit() {
	if (!this->m_Group.IsGrouped()) {
		...
		if (sMoney::IsCash(strCash) && (strAmount.Compare("coins") || nAmount != -1)) {
			sMoney MoneyToSplit;
			try {
...
void CCharacter::Track() {
	if (!InPosition(POS_FIGHTING) && InPosition(POS_STANDING) && !m_pTrackInfo->m_TrackList.IsEmpty()) {
		int nTrackPulse;
...
void CCharacter::DoExchange() {
	...
	try {
		if ((nAmount = CurrentCommand.GetWordAfter(1).MakeInt()) != -1) {
			...
			if (sMoney::IsCash(str1) && sMoney::IsCash(str2) && !str1.Compare("coins") && !str2.Compare("coins")) {
				int nLeft;
				switch (str1[0]) {
				case 'c':
				case 'C':
...
void CCharacter::DoAutoFlee() {
	...
	if ((m_nCurrentHpts <= m_nWimpy) && CanFight() && IsFighting() && !HasMaster()) {
		DoFlee();
	}
}
...
void CCharacter::GodChange() {
	CString strTarget(CurrentCommand.GetWordAfter(1));
	const char *s =
			"syntax: gchange <player name> <attribute | namne | hitpoints | mana | moves | frags | alignment> <value>\r\n"
	...
	if (!pTarget) {
		...
		if (strAtt.Compare("frags")) {
			...
			if (strAtt.Compare("strength")) {
				...
				pTarget->m_nCurrentHpts =
						pTarget->m_nCurrentHpts > pTarget->m_nMaxHpts ? pTarget->m_nMaxHpts : pTarget->m_nCurrentHpts;
			} else if (strAtt.Compare("mana")) {
				nValue = strValue.MakeInt();
				nValue = nValue < 1 ? 1 : nValue;
				pTarget->m_nMaxManaPts = nValue;
				pTarget->m_nManaPts =
						pTarget->m_nManaPts > pTarget->m_nMaxManaPts ? pTarget->m_nMaxManaPts : pTarget->m_nManaPts;
			} else if (strAtt.Compare("moves")) {
				nValue = strValue.MakeInt();
				nValue = nValue < 1 ? 1 : nValue;
				pTarget->m_nMaxMovePts = nValue;
				pTarget->m_nMovePts =
						pTarget->m_nMovePts > pTarget->m_nMaxMovePts ? pTarget->m_nMaxMovePts : pTarget->m_nMovePts;
			} else {
				SendToChar(s);
				return;
			}
			strToChar.Format("You change %s's %s to %d.\r\n", pTarget->GetName().cptr(), strAtt.cptr(), nValue);
			SendToChar(strToChar);
	...
	if (this->StatCheck(LUCK)) {
		...
		if (pCon == NULL) {
			...
			if ((pObj = pCon->IsContentsAffectedBy(CObject::OBJ_AFFECT_HIDDEN))) {
				pObj->RemoveAffect(CObject::OBJ_AFFECT_HIDDEN);
				str.Format("You find %s in the %s", (const char *) pObj->GetObjName(this),
...
skill_type CCharacter::GetSkill(short nSkill) const {
	...
	case SKILL_KICK:
	case SKILL_BASH:
	case SKILL_HEAD_BUTT:
	case SKILL_BACKSTAB:
	case SKILL_OFFENSE:
	case SKILL_CIRCLE:
	case SKILL_QUICK_STRIKE:
...
bool CCharacter::EnoughExpForNextLevel(bool bLastMortalLevelRestrict) {
	...
	if ((GetLevel() < LVL_LAST_MORTAL || bLastMortalLevelRestrict == false) && GetLevel() < LVL_IMMORT
