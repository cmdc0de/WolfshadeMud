...
protected:
	CCaster(CCharIntermediate *ch) : CCharacter(ch) 
	{
	...
	CCaster(CMobPrototype &mob,CRoom *pPutInRoom) : CCharacter(mob,pPutInRoom) 
	{
		m_SpellsBeingPrepared.SetDeleteData();
		m_SpellsPrepared.SetDeleteData();
		m_CurrentSpell.Empty();
...
void CCaster<CasterClass>::LoadSpells()
{
	...
	std::ifstream SpellFile(strFile,std::ios::binary | std::ios::in);
	if(!SpellFile)
	{
		return;
	}
	...
	for(i=0;i<SavedSpells.GetMaxSpellsPrepared() 
		&& !SavedSpells.GetPreparedSpell(i).IsEmpty();i++)
	{
	...
	for(i=0;i<SavedSpells.GetMaxSpellsPreparing() 
		&& !SavedSpells.GetPreparingSpell(i).IsEmpty();i++)
	{
		if((pSpell=this->GetSpellInfo(CString(SavedSpells.GetPreparingSpell(i).GetName()))))
		{
			m_SpellsBeingPrepared.Add(
				new CPreparing<CasterClass>(
				pSpell,
				SavedSpells.GetPreparingSpell(i).GetTimeLeft(),
...
void CCaster<CasterClass>::Save(bool bDidJustDie)
{
	if(!this->IsNPC())
	{
		CString strFile(SPELL_SAVE_PREFIX);
		strFile+=m_strName;
		std::ofstream SpellFile(strFile,std::ios::binary | std::ios::out);
		if(!SpellFile)
		{
...
bool CCaster<CasterClass>::HasCommand()
{
	...
	if((InPosition(POS_PREP)) && !m_SpellsBeingPrepared.IsEmpty())
	{
		...
		if(pSpellPreparing && 
			((--(*pSpellPreparing) <= 0) ||
			(IsAffectedBy(CCharacter::AFFECT_MEDITATE) &&
			pSpellPreparing->TimeToCheckMed() && 
			(DIE(100) <= GetSkill(SKILL_MEDITATE)))))
		{
			...
			strToChar.Format("You finish preparing %s.\r\n",
				(char *)pSpellPreparing->GetSpell()->GetSpellName());
			SendToChar(strToChar);
		...
		else 
		{
			if((m_lLagTime-1) && !((m_lLagTime-1)%(CMudTime::PULSES_PER_CASTING_ROUND)))
			{
				...
				else
				{
					...
					strCastMsg.Format(CastingMsg[nIndex],(char *)m_CurrentSpell.m_pSpell->GetSpellName());
...
void CCaster<CasterClass>::DoCast(const CInterp *pInterp, bool bForceCast)
{
	...
	else
	{
		if(this->CastingTargetInRoom())
		{
			...
			strTemp.Format("utters the words '%s'\r\n",(char *)m_CurrentSpell.m_pSpell->GetSpellName());
			strToRoom += strTemp;
...
void CCaster<CasterClass>::UpDatePosition(UINT nPosition, bool bSendStr)
{
	...
	//	give em mana back
	if((InPosition(POS_PREP)) && (!(nPosition & POS_RESTING)) &&
		(!(nPosition & POS_PREP)) && !m_SpellsBeingPrepared.IsEmpty())
	{
		CPreparing<CasterClass> *pPreping;
...
void CCaster<CasterClass>::GetSpellsPreparedAndPreparing(CString & strSpells)
{
	...
	while(pos)
	{
		...
		if((short)(*pPreped)>1)
		{
			strToChar.Format("%s(%d) %s\r\n",(char *)strToChar,
				(short)(*pPreped),(char *)pPreped->GetSpell()->GetColorizedName());
		}
		else
		{
			strToChar.Format("%s%s\r\n",(char *)strToChar,
				(char *)pPreped->GetSpell()->GetColorizedName());
		}
	}
	strToChar += "\r\nCurrently Preparing: \r\n";
	...
	while(pos)
	{
		...
		strToChar.Format("%s%s (%d) (%d)\r\n",(char *)strToChar,
			(char *)pPreparing->GetSpell()->GetColorizedName(),
			pPreparing->GetSecondsLeft(),
			nTotalTime);
	}
	strSpells += strToChar;
}
	...
	while(pos)
	{
		...
		else
		{
			...
			strToChar.Format("%sYou don't have enough mana to prepare %s.\r\n",
				(char *)strToChar,
				(char *)pPreparing->GetSpell()->GetSpellName());
...
{
	short nPrepTime = (short) pSpell->GetPrepTime() + m_pAttributes->GetIntPrepTimeAdjust() - ((GetLevel() - pSpell->GetMinLevel())/2);
...
void CCaster<CasterClass>::PrepareSpell(CString strSpell)
{
	...
	else
	{
		strToChar.Format("The preparation of %s will cost you %d mana points.\r\n",
			(char *)pSpell->GetColorizedName(),pSpell->GetMana());
		//reduce mana
		UpDatePosition(POS_PREP);
...
void CCaster<CasterClass>::DoPrepare()
{
	...
	if(!strSpell.IsEmpty() && InPosition(POS_RESTING)
		&& InPosition(POS_PREP))
	{
	...
	else if(!strSpell.IsEmpty() && InPosition(POS_RESTING)
		&& !InPosition(POS_PREP))
	{
		RePrep(true);
		PrepareSpell(strSpell);
	...
	//ones they have
		...
		!InPosition(POS_FIGHTING) && !InPosition(POS_PREP))
	{
		RePrep();
	}
