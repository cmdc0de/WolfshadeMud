...
void CWorld::BootWorld()
{
	try
	{
		ReadInVirtualRooms();
		ReadRealRooms();
		MudLog << "size of a room is : " << sizeof(CRoom) << endl;
	}
	catch(CError *Er)
	{
		ErrorLog << Er->GetMessage() << endl;
...
void CWorld::ReadInVirtualRooms()
{
	...
	CAscii Virtualtypes(VIRTURALINDEX,ios::in|ios::nocreate);
	if(!Virtualtypes)
	{
	...
	CAscii VirtualWorld(VIRTUALWORLD,ios::in|ios::nocreate);
	if(!VirtualWorld)
	{
	...
	for(int y=0;y<VIRTUALHEIGHT;y++)
	{
		for(int x=0;x<VIRTUALWIDTH;x++)
		{
			...
			if(VirtualPrototypes.Lookup(temp.MakeLong(),pRoom))
			{
				...
				SetVirtualDoors(pNewRoom, VirtualWorld.AsciiConvert(temp));
			}
			else
			{
...
void CWorld::ReadRealRooms()
{
	...
	ifstream Index(REALINDEX,ios::in|ios::nocreate);
	if(!Index)
	{
		Error.Format("File won't open: %s", (char *)REALINDEX);
		CError Err(CRITICAL_ERROR,Error);
		throw &Err;
	}
	...
	while(Index.good())
	{
		...
		Index >> temp;
		strFile+=temp;
		CAscii CurrentFile(strFile,ios::in|ios::nocreate);
		if(!CurrentFile)
		{
			Error.Format("File would not open! %s",(char *)temp);
			CError Err(CRITICAL_ERROR,Error);
			throw &Err;
		}
...
void CWorld::BootShops(const CObjectManager *pObjManager,const CMobManager *pMobManager)
{
	...
	ifstream Index(SHOPINDEX,ios::in|ios::nocreate);
	if(!Index)
	{
	...
	while(Index.good())
	{
		...
		Index >> temp1;
		temp+=temp1;
		CAscii CurrentFile(temp,ios::in|ios::nocreate);
		if(!CurrentFile)
		{
			Error.Format("File would not open! %s",(char *)temp);
			CError Err(NON_CRITICAL_ERROR,Error);
			throw &Err;
		}
	...
	MudLog << "size of a shop room is " << sizeof(CShopRoom) << endl;
	Index.close();
}
...
void CWorld::BuildMap()
{
	...
	for(register int i=0;i<VIRTUALHEIGHT;i++)
	{
		for(register int j=0;j<VIRTUALWIDTH;j++)
		{
