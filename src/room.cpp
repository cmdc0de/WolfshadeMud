...
void CRoom::InitStatics()
{
	...
	for(register int i=0;i<TOTAL_ROOM_TYPES;i++)
	{
...
short CRoom::LocateChar(CCharacter *pCh, CLinkList<CRoom> &UsedLL, short nDistance)
{
	...
	for(register int i = 0;i<NUM_OF_DOORS && nDistance!=0;i++)
	{
		//makes sphere
		for(register int j=0;j<nDistance;j++)
		{
...
CRoom::CRoom()
{
	...
	for(register int i=0;i<NUM_OF_DOORS;i++)
		m_Doors[i]=NULL;
	m_nRoomSize = 0;
	m_nRoomType = 0;
	m_lVnum = -1;
...
void CRoom::ReadRoom(CAscii &Infile)
{
	...
	istrstream one(temp,MAX_FILE_STRING_SIZE);
	one >> m_lVnum;
	if(!one.good())
	{
	...
	for(i=0;i<TOTAL_ROOM_AFFECTS;i++)
	{
		if(IsAffectedBy(i))
		{
			strAffects.Format("%s %s",
				(char *)strAffects,
				(char *)m_AffectInfo[i].m_strAffectName);
		}
	}
	...
	while(pos)
	{
		...
		if(pch!=ch && pch!=pch2 && !pch->InPosition(POS_SLEEPING))
		{
			...
			if(ch && pch2)
			{
				...
				strToChar.Format(strToRoom,(char *)ch->GetRaceOrName(pch),(char *)pch2->GetRaceOrName(pch));
				pch->SendToChar(strToChar);
			}
			else if(ch)
			{
				strToChar.Format(strToRoom,(char *)ch->GetRaceOrName(pch));
				pch->SendToChar(strToChar);
			}
			else
			{
	...
	while(posChar)
	{
			...
			&& pCh->CanSeeInRoom(pCurrentPerson->GetRoom())==CCharacter::SEES_ALL)
		{
				...
				&& pCurrentPerson->IsInRoom(pCurrentPerson->IsFighting()))
			{
				...
				{
					strTemp.Format(" fighting %s.",(char *)pCurrentPerson->IsFighting()->GetRaceOrName(pCh));
					strInfo += strTemp;
				}
			}
			...
			if(pCurrentPerson->IsNPC())
			{
				if(pCurrentPerson->InPosition(pCurrentPerson->GetDefaultPos()) && !pCurrentPerson->IsFighting())
				{
					strLook.Format("%s%s%s\r\n",(char *)strLook, 
						(char *)pCurrentPerson->GetDefaultPosDesc(),
						(char *)strInfo);
				}
				else
				{
					strLook.Format("%s%s %s%s\r\n",(char *)strLook, 
						(char *)pCurrentPerson->GetName(),
			...
			else if(pCurrentPerson->IsAffectedBy(CCharacter::AFFECT_FUGUE_PLANE))
			{
				...
				(char *)GVM.GetColorRaceName(pCurrentPerson->GetRace()),
				(char *)pCurrentPerson->GetName());
			}
			else
			{
				if(pCh->CanSeeName(pCurrentPerson))
				{
					strLook.Format("%s%s (%s) [%s] %s %s%s\r\n",(char *)strLook, 
						(char *)pCurrentPerson->GetRaceOrName(pCh),
						GVM.GetColorRaceName(pCurrentPerson->GetRace()),
						(char *) CCharacterAttributes::GetSizeName(pCurrentPerson->GetSize()),
						(char *)pCurrentPerson->GetTitle(),
						pCurrentPerson->GetPositionNames(false), 
						(char *) strInfo);
				}
				else
...
void CRoom::SendToRoom(CString strFormat, const CCharacter * pCh, CString strForRoom)
{
	...
	while(pos)
	{
		...
		if(pch!=pCh && !pch->InPosition(POS_SLEEPING))
		{
			...
			strToChar.Format(strFormat,(char *)str,(char *) strForRoom);
			pch->SendToChar(strToChar);
		}
	}
}
...
void CRoom::SendToRoom(CString strFormat, const CCharacter * pCh, const CCharacter * pCh2, CString strOne, CString strTwo)
{
	...
	while(pos)
	{
		...
		{
			strToChar.Format(strFormat,
				(char *)pCh->GetRaceOrName(pch),
				(char *)pCh2->GetRaceOrName(pch),
				(char *)strOne,
				(char *)strTwo);
			pch->SendToChar(strToChar);
		}
...
CString CRoom::DoRoomLook(CCharacter *pLooker)
{
	...
	if(nVision==CCharacter::SEES_ALL)
	{
		...
		{
			strToChar.Format("%s\r\n\r\n%s\r\n%s",(char *)strToChar,
				(char *)m_strRoomName,(char *)GetExitString(pLooker));
		}
		else
		{
			strToChar.Format("%s\r\n\r\n%s\r\n%s\r\n%s",(char *)strToChar,
...
void CRoom::Scan(CCharacter * pCh, CString & str)
{
	...
	for(register int i = 0;i<NUM_OF_DOORS;i++)
	{
		...
		for(register int j = 0;j<3 && pRoom!=NULL;j++)
		{
				...
				if(pCh->CanSeeChar(pTarget))
				{
					...
					if(nVision==CCharacter::SEES_ALL)
					{
						str.Format("%sYou see %s %s %s\r\n",
							(char *)str,
							(char *)pTarget->GetRaceOrName(pCh),
							strScan[j],
							strDir[i]);
					}
					else if(nVision==CCharacter::SEES_RED_DOTS)
	...
	for(i=0;i<NUM_OF_DOORS;i++)
	{
		if(m_Doors[i]!=NULL)
		{
			str.Format("%s%s\t%s\r\n",
				(char *)str,
				strDir[i],
				(char *)m_Doors[i]->m_pToRoom->m_strRoomName);
		}
	}
}
...
CString CRoom::GetMap(const CCharacter *pLooker)
{
	...
	{
		strMap.Format("\r\n\t&i &n%s&i &n\r\n\t%s%s%s\r\n\t&i &n%s&i &n",
			(char *)(m_Doors[NORTH] ? m_RoomColors[(m_Doors[NORTH]->m_pToRoom->m_nRoomType)] : m_strBlock),
			(char *)(m_Doors[WEST] ? m_RoomColors[(m_Doors[WEST]->m_pToRoom->m_nRoomType)] : m_strBlock),
			"&WO&n",
			(char *)(m_Doors[EAST] ? m_RoomColors[(m_Doors[EAST]->m_pToRoom->m_nRoomType)] : m_strBlock),
			(char *)(m_Doors[SOUTH] ? m_RoomColors[(m_Doors[SOUTH]->m_pToRoom->m_nRoomType)] : m_strBlock));
	}
	else 
	{
		...
		const CRoom *pRoom;
		for(register int i=nStartY;i<=nEndY;i++)
		{
			strMap+="\r\n\t";
			for(register int j=nStartX;j<=nEndX;j++)
			{
