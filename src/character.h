...
private:
	...
	public:
		sAffectInfo() {m_pFncApply = NULL;m_pFncRound = NULL;m_nRateOfAffect = 0;m_bAcumTime=m_bAcumValue=m_bReferenceAffect=false;}
		...
		bool IsReferenceAffect() {return m_bReferenceAffect;}
	};
	struct sMaster
	{
		friend class CCharacter;
	...
	protected:
		void Add(CCharacter *pCh) {m_MasterOf.Add(pCh);}
	public:
		sMaster() {m_pMaster = NULL;m_nTimeIAmMastered=-1;}
		void SetMaster(CCharacter *pMaster, CCharacter *pMasterie, int nTime, bool bMakeFollow = true);
		bool IsMasterOf(CCharacter *pCh) {return m_MasterOf.DoesContain(pCh);}
		void Remove(CCharacter *pCh);
		bool IsAMaster() {return !m_MasterOf.IsEmpty();}
		bool HasMaster() const {return (m_pMaster!=NULL);}
		CCharacter *GetMaster() const {return m_pMaster;}
	...
	struct sFollower
	{
		...
		bool IsFollowing() {return m_pCharacterBeingFollowed!=NULL;}
		sFollower()	{m_pCharacterBeingFollowed = NULL;}
		bool HasFollowers() {return !m_Followers.IsEmpty();}
		bool HasFollower(CCharacter *pCh) {return m_Followers.DoesContain(pCh);}
		void Remove(CCharacter *pCh);
		void CleanUp(CCharacter *pOwner);
	};
	friend sFollower;
	struct sConsent
	{
		...
		sConsent(){m_pCharYouConsented = NULL;}
		CCharacter *GetConsented() const {return m_pCharYouConsented;}
		void CleanUp(CCharacter *pOwner);
	};
	friend sConsent;
	struct sGroup
	...
	protected:
		...
		{m_pGroup=new CList<CCharacter *>();m_pGroup->Add(pHead);}
		bool IsHead(CCharacter *pCh) {return m_pGroup ? m_pGroup->GetFirst()==pCh : false;}
		CList<CCharacter *> *m_pGroup; //if null not grouped
		CList<CCharacter *> *NullGroup();
	public:
		sGroup(const sGroup &copy); //broke
		sGroup() {m_pGroup=NULL;}
		void Remove(CCharacter *pCh);
		void SetGroup(CList<CCharacter *> *pGroup){m_pGroup=pGroup;}
		bool IsInGroup(CCharacter *pCh) 
		{return (m_pGroup ? m_pGroup->DoesContain(pCh) : false);}
		bool IsGrouped() {return m_pGroup!=NULL;}
	...
	struct sPageInfo
	{
		CString m_strPage;
		int m_nReturnsSent;
		sPageInfo() {m_nReturnsSent = 0;}
		void Empty() {m_strPage.Empty();m_nReturnsSent = 0;}
	};
...
protected:
	...
	//inline publics
public:
	void DoInnate() {m_pAttributes->m_pRaceReference->DoInnate(this,CurrentCommand.GetWordAfter(1,true));}
	stat_type GetAgility() {return m_pAttributes->GetStat(AGI);}
	stat_type GetConstitution() {return m_pAttributes->GetStat(CON);}
	inline const char *GetRaceWarName();
	bool IsInRoom(const CCharacter *pCh) const {return (pCh==NULL ? false : pCh->IsDead() ? false : pCh->GetRoom()==this->GetRoom());}
	void AdjustAC(short nAdj) {m_nAC = (m_nAC+nAdj>100) ? 100 : (m_nAC+nAdj<-100) ? -100 : m_nAC+nAdj;}
	bool ShouldMakeCorpse() {return m_bMakeCorpseOnDeath;}
	void SetNoCorpse() {m_bMakeCorpseOnDeath = false;}
	inline void AddCommand(CString str);
	inline short GetLoadPct(const CObject *pBeingDraged=NULL) const;
	bool HasInnate(int nInnate) const {return m_pAttributes->m_pRaceReference->HasInnate(nInnate);}
	bool HasMaster() const {return m_Master.HasMaster();}
	bool CommandBufEmpty() {return (m_pDescriptor!=NULL ? m_pDescriptor->CommandBuf.IsEmpty() : false);}
	int CurrentlyMasterOf() const {return m_Master.m_MasterOf.GetCount();}
	void SetMaster(CCharacter *pCh, int nTime, bool bMakeFollow=true) {m_Master.SetMaster(pCh,this,nTime,bMakeFollow);}
	CCharacter *GetMaster() {return m_Master.GetMaster();}
	void LagChar(long lLag) {m_lLagTime+=lLag;}
	bool IsLagged() {return m_lLagTime>0;}
	const CCharacter *GetConsented() const {return m_Consent.GetConsented();}
	sGroup &GetGroup() {return m_Group;}
	CCharacter *IsFollowing() {return m_Follow.m_pCharacterBeingFollowed;}
	inline bool CanSeeName(const CCharacter *pCh) const;
	inline bool CanSee(CObject *pObj) const;
	bool CanFight() {return (!InPosition(POS_INCAP) && !InPosition(POS_DEAD) && !InPosition(POS_MORTALED) && !InPosition(POS_KOED));}
	bool IsDead() const {return InPosition(POS_DEAD);}
	CRoom *GetRoom() const {return m_pInRoom;}
	CCharacter *IsFighting() {return m_pFighting;}
	//this might need to be CDescriptor instead of player descriptor
	void SetDescriptor(sPlayerDescriptor *pDesc) {m_pDescriptor = pDesc;}
	inline sPlayerDescriptor *KillSocket();
	inline void AssertFighting(CCharacter *pCh);
	bool IsLinkDead() const {return m_pDescriptor ? m_pDescriptor->IsLinkDead() : true;}
	bool ShouldCutLink() {return m_pDescriptor ? m_pDescriptor->IsLinkDead() : false;}
	bool ShouldRent() {return m_pDescriptor ? m_pDescriptor->ShouldRent() : false;}
	inline bool IsPlaying();
	sPlayerDescriptor * GetDescriptor() {return (sPlayerDescriptor *)m_pDescriptor;}
	short int GetSex() {return m_pAttributes->AffectedSex;}
	inline void RemovePosition(UINT nRemove);
	bool MakeSavingThrow(short nSave) {return m_pAttributes->MakeSavingThrow(nSave);}
	...
	bool IsBeingSpied() const {return m_pSpiedby!=NULL;}
	bool IsSpying() const {return m_pSpyingOn!=NULL;}
//non inlined
public:
...
protected:
	...
	void DoSwitch();
	bool IsSwitched() const {return m_pSwitchedTo!=NULL;}
...
protected: //inlined
	short GetMaxWeaponPositions() {return m_pAttributes->m_pRaceReference->GetMaxWeapons();}
	short GetWeaponPosition(short n) {return m_pAttributes->m_pRaceReference->GetWeaponPosition(n);}
	bool IsKillFrag(const CCharacter *pCh) const {return (!CanSeeName(pCh) && pCh->GetLevel()>=MIN_FRAG_LEVEL && ((this->GetLevel()-MAX_FRAG_LEVEL_DIFF)<=pCh->GetLevel()));}
	void SendMotd() {SendToChar(MOTD);}
	//virtual functions
protected:
	virtual void DoBreakMastersHold() {return;} //nothing special for pc's
	virtual void InitializeCharacter();
	virtual void ResetCharVars();
	virtual int GetExtraExp() {return 0;} //only used for npc
	...
	virtual void SetSwitched(CCharacter *pCh) {return;}
	...
	virtual bool IsSwitchedTo() {return false;}
	virtual bool IsTeacher() const {return true;}
public:
	...
	virtual bool CheckVoid() {return (IsGod() ? ++m_lVoidTime>=GOD_VOID_TIME : ++m_lVoidTime>=CHARACTER_VOID_TIME);}
	virtual bool IsGod() const {return m_nLevel>=LVL_IMMORT;}
	virtual int AdvanceLevel(bool IncreaseLevel,bool bSendMsg = true,bool bIsFollower = false);
	virtual sMoney GetTrainingCosts(const CCharacter *pToBeTrained,short nSkill,bool bIsSkill) const {sMoney Mon;return Mon;} //pc's can figure out what they pay each other
	...
	virtual UINT GetDefaultPos() {assert(0);return 0;}
	virtual CString GetDefaultPosDesc(){assert(0);return CString();} //only for npcs
	...
	//defined in CCaster
	virtual void LoadSpells() {return;}
	//defined in CNPC
	virtual void AddMemory(CCharacter *pCh) {return;}
	virtual void DoAI(const CInterp *pInterp) {return;}
	virtual void RemoveFromMemory(CCharacter *pCh) {return;}
	virtual void Track();
	//static const
public:
...
inline void CCharacter::AddCommand(CString str) 
{
	...
	{
		m_pDescriptor->CommandBuf.Format("%s\r\n%s",(char *)str,(char *)m_pDescriptor->CommandBuf);
	}
}
...
inline void CCharacter::RemovePosition(UINT nRemove)
{
	...
	if(nRemove & POS_STUNNED)
	{
		SendToChar("The world stops spinning.\r\n");
	}
}
...
private:
	...
	UINT m_nWearBit;
protected:
	CEqInfo(CObject *pObj,UINT nWearBit) {m_pObj = pObj;m_nWearBit = nWearBit;}
	CEqInfo(const CEqInfo &copy) {m_pObj = copy.m_pObj;m_nWearBit = copy.m_nWearBit;}
	void Empty() {m_pObj = NULL;m_nWearBit = 0;}
public:
	CEqInfo() {m_pObj = NULL;m_nWearBit = 0;}
	UINT GetWearBit() {return m_nWearBit;}
	bool IsWorn(UINT nPos) {return (nPos & m_nWearBit) ? true : false;}
	CObject *operator ->() {return m_pObj;}
	bool IsEmpty() {return m_pObj==NULL;}
	CObject * GetObject() {return m_pObj;}
};
#endif