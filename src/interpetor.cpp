...
CInterp::CInterp(CGame *pGame)
{
	...
	BuildCommands();
	BuildHelp();
	BugFile.open(BUG_FILE,ios::out | ios::app);
}
	...
	if(pTarget!=NULL && pTarget!=pCh)
	{
		if(pTarget->InPosition(POS_SLEEPING))
		{
			...
			strToChar.Format("%s wakes you up.\r\n",
				(char *)pCh->GetRaceOrName(pTarget));
	...
	else
	{
		...
		else
		{
			...
			strToGod.Format("You transport %s to room %u\r\n",
				(char *)pTarget->GetName(),pRealRoom->GetVnum());
		...
		else
		{
			...
			if(nLevel == pTarget->m_nLevel)
			{
				strToChar.Format("%s is already %d.",
					(char *)pTarget->GetName(),pTarget->m_nLevel);
				ch->SendToChar(strToChar);
			}
			else if(nLevel < pTarget->m_nLevel)
			{
				for(register int i = pTarget->m_nLevel;
					i > nLevel;i--)
				{
				...
				strToChar.Format("You lower %s's level to %d.\r\n",
					(char *)pTarget->GetName(),pTarget->GetLevel());
				ch->SendToChar(strToChar);
			}
			else
			{
				for(register int i = pTarget->m_nLevel;i<nLevel;i++)
				{
		...
		if(pTarget==pCh)
		{
			...
			if(pCh->m_Consent.m_pCharYouConsented)
			{
				...
				strToChar.Format("You no longer consent %s.\r\n",
					(char *)pCh->m_Consent.m_pCharYouConsented->GetRaceOrName(pCh));
				pCh->SendToChar(strToChar);
...
void CInterp::BuildHelp()
{
	...
	m_HelpFile.open(m_strHelpFileName,ios::in | ios::nocreate);
...
void CInterp::Help(CCharacter *pCh)
{
	...
	else
	{
		...
		{
			m_HelpFile.seekg(pHelpInfo->m_lStartPos,ios::beg);
			do
			{
		...
		if(nValue!=-1)
		{
			...
			if(nSkillSpell != -1)
			{
				...
				strMsg.Format("You set %s's skill in %s to %d.\r\n",
					(char *)pTarget->GetName(),
					(char *)CCharacterAttributes::FindSkill(nSkillSpell),
					nValue);
...
void CInterp::Who(CCharacter *pToChar)
{
	...
	strChars.Format("%s&r\r\nTotal number of players: %d&n\r\n",(char *)strChars,nNumOfPlayers);
	if(pToChar->IsGod())
	{
		strChars.Format("%sTotal mobs: %d\r\n", (char *)strChars, nTotalMobs);
	}
	strGods.Format("%s\r\n%s",(char *)strGods,(char *)strChars);
			...
			if(!pTarget->IsNPC())
			{
				...
				else
				{
					strToChar.Format("%s is already being spied by %s.\r\n",
						(char *)pTarget->GetName(),(char *)pTarget->m_pSpiedby->GetName());
					pCh->SendToChar(strToChar);
				}
			}
			else
			...
			(!bGod && !pCom1->IsGodCommand()))
		{
			if(++nCounter==3)
			{
				str.Format("%s%-15s%-15s%-15s\r\n",
					(char *)str,
					(char *)pCom3->CommandName,
					(char *)pCom2->CommandName,
					(char *)pCom1->CommandName);
				nCounter=0;
			}
			else if(nCounter==2)
			{
...
void CInterp::DoPetition(CCharacter *pCh)
{
	...
	if(pCh->IsNPC())
	{
		if(pCh->m_Master.HasMaster())
		{
			...
			(char *)pCh->m_Master.GetMaster()->GetName(),
			(char *)str);
		...
		while(pos)
		{
			...
			if(!p->IsNPC())
			{
				strInfo.Format("%s%-15s%-10s%-40s\r\n",
					...
					p->m_pDescriptor==NULL ? "" : (char *)((sPlayerDescriptor *)p->m_pDescriptor)->Host);
			}
		}
	}
