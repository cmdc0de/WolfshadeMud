...
CImpReference::~CImpReference()
{
	//do nothing
}

CImpReference::CImpReference() : CRacialReference()
{
	SetAll();
	m_nSize = SIZE_TINY;
	...
	m_nWearPositions [7]= WEAR_POS_ABOUT_BODY;
	m_nWearPositions [8] = WEAR_POS_ON_BACK;
	m_nWearPositions [9]= WEAR_POS_WINGS;
...
void CImpReference::SetCharisma()
{
...
void CImpReference::SetIntellegence()
{
...
void CImpReference::SetConstitution()
{
	...
	m_sConstitution[7] = sConstitution(1,1,10); //70-79
	m_sConstitution[8] = sConstitution(1,2,20); //80-89
	m_sConstitution[9] = sConstitution(2,3,30); //90-99
	m_sConstitution[10] = sConstitution(3,4,40); //100-109
	m_sConstitution[11] = sConstitution(4,4,40); //110-119
	m_sConstitution[12] = sConstitution(5,4,42); //120
}

void CImpReference::SetInnateRace()
{
	m_sInnateRace.m_nInnate_ablities = INNATE_ULTRAVISION |
		INNATE_DAY_SIGHT | INNATE_FLY | INNATE_BITE;
	m_sInnateRace.m_nMinHeight = 12;
	m_sInnateRace.m_nMaxHeight = 24;
	m_sInnateRace.m_nMinWeight = 10;
	m_sInnateRace.m_nMaxWeight = 100;
	m_sInnateRace.m_nMinAge = 1;
	m_sInnateRace.m_nMaxAge = 30;
...
void CImpReference::SetStrength()
{
	...
	m_sStrength[4] = sStrength(1,50,0); //40-49
	m_sStrength[5] = sStrength(1,70,0); //50-59
	m_sStrength[6] = sStrength(1,90,0); //60-69
	m_sStrength[7] = sStrength(2,110,0); //70-79
	m_sStrength[8] = sStrength(3,130,10); //80-89
	m_sStrength[9] = sStrength(4,160,20); //90-99
	m_sStrength[10] = sStrength(5,190,30); //90-99
	m_sStrength[11] = sStrength(5,220,30); //110-119
	m_sStrength[12] = sStrength(6,250,35); //120
}

void CImpReference::SetDexterity()
{
