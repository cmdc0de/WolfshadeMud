...
public:
	...
	int buffer_size() const { return epptr() - pbase(); }
	int buffered_chars() const { return pptr() - pbase(); }
	const char *GetBuffer() const {return pbase();}
	bool empty() const { return pptr() == pbase(); }
	CWolfshadeBuf *open(const char *name,int n, int p) {return ((CWolfshadeBuf *)std::filebuf::open(name,n,p));}
...
private:
	bool m_bToScreen;
protected:
	CWolfshadeBuf* rdbuf() const { return (CWolfshadeBuf*) std::ios::rdbuf(); }
	void CheckTime()
	{
		if(rdbuf()->empty())
		{
...
public:
	...
	CWolfshadeLog(bool bToScreen=true) : std::ostream(new CWolfshadeBuf()) {m_bToScreen = bToScreen;}
...
public:
	CWolfshadeLog &operator<<(CWolfshadeLog & (_f)(CWolfshadeLog &r)){(_f)(*this);return *this;}
	CWolfshadeLog &operator<<(const char *a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &operator<<(const int a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &operator<<(char a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &operator<<(long a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &operator<<(float a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &operator<<(double a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &operator<<(unsigned int a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &operator<<(CString a) {CheckTime();if(m_bToScreen){ std::cout << a;}std::ostream::operator<<(a);return *this;}
	CWolfshadeLog &end()
	{
		if(m_bToScreen)
			std::cout <<  std::endl;
...
public: 
        ...
        friend CWolfshadeLog& operator<<(CWolfshadeLog& s, const WSMMANIP & sm) { (*(sm._fp))(s,*sm._tp); return s; } 
private:        
