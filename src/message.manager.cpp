...
void CMessageManager::SendHit(CCharacter * pAttacker, CCharacter * pDefender, short nWeaponType, short nDam) const
{
	...
	if(pDefender->IsDead())
	{
		Msg.Format(m_AttackMessages[TO_DAMAGER_DEATH],
			(char *)strHit,m_WeaponTypes[nWeaponType],
			(char *) pDefender->GetRaceOrName(pAttacker));
		pAttacker->SendToChar(Msg);
		Msg.Format(m_AttackMessages[TO_DAMAGEE_DEATH],(char *)pAttacker->GetRaceOrName(pDefender),
				CCharacterAttributes::GetPronoun(pAttacker->GetSex()),(char *)strHit,
				m_WeaponTypes[nWeaponType]);
		pDefender->SendToChar(Msg);
		Msg.Format(m_AttackMessages[TO_ROOM_DEATH],(char *)strHit,
				m_WeaponTypes[nWeaponType]);
		Msg2 += Msg;
		Msg2 += "%s.\r\n";
	...
	{
		Msg.Format(m_AttackMessages[TO_DAMAGER_HIT],(char *)strHit,m_WeaponTypes[nWeaponType],(char *)strDam,
			(char *) pDefender->GetRaceOrName(pAttacker));
		pAttacker->SendToChar(Msg);
		Msg.Format(m_AttackMessages[TO_DAMAGEE_HIT],(char *)pAttacker->GetRaceOrName(pDefender),
			(char *)strHit,m_WeaponTypes[nWeaponType],(char *)strDam);
		pDefender->SendToChar(Msg);
		Msg.Format(m_AttackMessages[TO_ROOM_HIT],(char *)strHit,m_WeaponTypes[nWeaponType],(char *)strDam);
		Msg2 += Msg;
		Msg2 += "%s\r\n";
		pDefender->GetRoom()->SendToRoom(Msg2,pAttacker,pDefender);
	}
...
void CMessageManager::SendMiss(CCharacter * pAttacker, CCharacter * pDefender, short nWeaponType) const
{
	...
	CString Msg;
	Msg.Format(m_AttackMessages[TO_DAMAGER_MISS],(char *)pDefender->GetRaceOrName(pAttacker),m_WeaponTypes[nWeaponType]);
	pAttacker->SendToChar(Msg);
	Msg.Format(m_AttackMessages[TO_DAMAGEE_MISS],(char *)pAttacker->GetRaceOrName(pDefender),
...
{
	CAscii SkillMsgFile(SKILL_MSG_FILE,ios::in | ios::nocreate);
...
void CMessageManager::BootMessageFile(char *strFile,int nMessages,CMap<CString, CMsg *> *&pMap, int nHas)
{
	CAscii MsgFile(strFile,ios::in | ios::nocreate);
...
void CMessageManager::SkillSaveMsg(CCharacter *pSaved, CCharacter *pAttacker, const char *strSkill) const
{
	...
	str.Format("You side step %s's %s.\r\n",
		(char *)pAttacker->GetRaceOrName(pSaved),
		strSkill);
	pSaved->SendToChar(str);
	str.Format("%s side steps your %s.\r\n",
		(char *)pSaved->GetRaceOrName(pAttacker),
		strSkill);
