...
const char *CObjectPrototype::MaterialNames[TOTAL_MATERIALS] = { "*Reserved*", "Wood", "Steel", "Spanish Steel",
		"Amantite", "Leather", "Gold", "Nickle" };

const char *CObjectPrototype::TypeNames[] = { "NONE", "FOOD", "LIGHT", "SCROLL", "WAND", "STAFF", "ARMOR",
		"DRINKCONTAINER", "CONTAINER", "FOUNTAIN", "WEAPON", "POTION", "PORTAL", "reserved", "TRANSPORT" };

const char *CObjectPrototype::AffectNames[] = { "NONE", "STRADD", "STR", "AGI", "DEX", "INT", "WIS", "CON", "CHA",
		"POW", "LUCK", "CLASS", "LEVEL", "AGE", "WIEGHT", "HEIGHT", "MANA", "HITPOINTS", "MOVE", "EXP", "AC", "HITROLL",
		"DAMROLL", "SAVE_PARA", "SAVE_ROD", "SAVE_BREATHE", "SAVE_SPELL" };
...
void CObjectPrototype::VStat(CString & ToChar) {
	static CString strFormat("Name: %s       Material: %s\r\n"
	...
	ToChar.Format(strFormat.cptr(), ObjName.cptr(), GetMaterialName(Material), Val0, Val1, Val2, Val3, Val4, Weight,
			Cost, GetTypeName(ObjType), ObjAlias.cptr(), ObjDesc.cptr());
	...
	while (pos) {
		...
		str.Format("%s %d\r\n", GetAffectName(pAff->Location), pAff->Affect);
		ToChar += str;
	}
}
