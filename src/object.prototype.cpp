...
void CObjectPrototype::ReadObj(CAscii &Infile)
{
	...
	while(temp!=(char *)"ENDOBJ" && Infile.good())
	{
		if(temp==(char *)"EXTRA")
		{
		...
		else if (temp==(char *) "AFFECTS")
		{
			Infile.getline(temp,MAX_FILE_STRING_SIZE);
			while(temp!=(char *)"ENDOBJ")
			{
				CString strNum;
...
void CObjectPrototype::VStat(CString & ToChar)
{
	...
	ToChar.Format(strFormat,(char *)ObjName,GetMaterialName(Material),
		Val0,Val1,Val2,Val3,Val4,Weight,Cost,
		GetTypeName(ObjType),(char *)ObjAlias,
		(char *)ObjDesc);
